...
Cambodia has seen strong growth but poverty remains. The study applies a multidimensional approach aligned with the Global MPI to capture deprivations beyond income, focusing on education, health, and living standards.
...
The aim is to integrate spatial and survey data using AI/ML to produce detailed poverty maps. This helps policymakers allocate resources efficiently and identify local vulnerabilities.
...
Prior research shows satellites and ML can help predict poverty, but integration with socioeconomic surveys for multidimensional poverty is limited. This study fills that gap.
...
A wide set of data was used: CSES for household info, EO data for environment and infrastructure, and building footprints to scale down predictions to household level.
...
The Random Forest algorithm was selected due to robustness and ability to process mixed data types. Models produce probability maps that can be aggregated at township, district, or province level.
...
Nightlights and population density best explain deprivation. Infrastructure access is also crucial. Indicators with spatial correlation (e.g., utilities) performed better than those tied to household-specific conditions.
...
Spatial maps show concentration of deprivation in remote, poorly connected regions. Urban and border areas with infrastructure show lower poverty.
...
While promising, ML struggles when data lack spatial correlation. Improved survey design can enhance integration. This hybrid approach shows potential for real-time, fine-grained poverty monitoring.
...
This work shows how AI and EO data complement traditional surveys to map multidimensional poverty. Future directions include advanced spatial analysis and deep learning models for better accuracy.