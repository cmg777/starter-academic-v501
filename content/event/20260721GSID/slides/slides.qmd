...
[AI slashes production cost — but not verification cost — so work migrates into the danger zone.]{.comment .fragment}
...
[The fix isn't to avoid AI. It's to design a workflow that separates *production* from *verification*.]{.comment .fragment}
...
[Grounded in your materials → far less hallucination, and every answer is traceable.]{.comment .fragment}
...
[One upload becomes many ways to learn — the AI podcasts and AI tutors behind metricsAI work exactly this way.]{.comment .fragment}
...
[Start on your laptop at a café; the lab is the same everywhere.]{.comment .fragment}
...
[Big geospatial data, no supercomputer — just a notebook and credentials.]{.comment .fragment}
...
[Lights capture activity at night; embeddings capture the built environment by day.]{.comment .fragment}
...
[A promising exploration → a project anyone can re-run and audit → GitHub.]{.comment .fragment}
...
[Claude Code + GitHub + Overleaf: code, results, and manuscript, all version-controlled.]{.comment .fragment}
...
[Every claim traces back to the commit that produced it.]{.comment .fragment}
