...
[The 89-quarter pre-period is the engine of inference — long enough to make the conformal and jackknife procedures informative, and to give every donor a credible placebo path.]{.comment}
...
[The two rules — non-negative, sum to one — keep the synthetic interpretable and forbid wild extrapolation. The per-quarter effect is then $\hat{\tau}_t = Y_{1t} - \sum_i \hat{\gamma}_i^{scm} Y_{it}$.]{.comment}
...
[Start from the plain SCM counterfactual (first term); add the imbalance the ridge outcome model $\hat{m}$ predicts (the parenthesis). If the SCM fit were perfect, the correction vanishes — augmentation only acts where there is imbalance to fix.]{.comment}
...
[The more we de-bias and balance, the larger the measured damage — the un-augmented SCM number is the *conservative* one.]{.comment}
