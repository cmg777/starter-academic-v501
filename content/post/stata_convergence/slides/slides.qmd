...
[The twist this talk delivers: beta convergence can be real while the distribution still widens.]{.comment}
...
[5,040 country-year rows. A balanced panel means the same 84 countries appear every year — no composition effects masquerading as convergence.]{.comment}
...
[$g_i$ is country $i$'s annualized growth; $\ln(y_{i,0})$ is its log starting income. A **negative** $\lambda$ means convergence — the poor grow faster.]{.comment}
...
[A total swing of 0.0079 — a complete reversal that the 60-year pooled regression silently averaged away.]{.comment}
...
[$\lambda$ depends on the window length $s$; the Barro–Sala-i-Martin **speed** $\beta$ does not. The **half-life** $\tau$ is the years to close half the gap.]{.comment}
...
[OLS needs parameters to enter *linearly*. Here $\beta$ is trapped in $e^{-\beta s}$ — so OLS can only recover the whole blob $\lambda$, then we back out $\beta$.]{.comment}
...
``` {.stata code-line-numbers="1|2|2"}
local s = 19
nl (outcome = {b0=1} - (1 - exp(-1*{b1=0.02}*`s'))/`s' * initial_inc), vce(robust)
```

[`{b1=0.02}` is $\beta$ with the 2% benchmark as its starting guess; the iterative solver then minimises squared residuals.]{.comment}
...
[The Barro–Sala-i-Martin equation is a *reparameterization* of the linear model — so the two routes must coincide.]{.comment}
