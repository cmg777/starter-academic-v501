...
(function () {
  ...
  function forest_plot(container) {
    ...
    const svg = d3.select(container).html("").append("svg")
      ...
      .attr("preserveAspectRatio", "xMidYMid meet");
    const colorMap = {
      // r_double_lasso labels (kept for reference compatibility)
      "First diff":    C.steel,
      "OLS (full)":    C.muted,
      "PSL":           "#66e5de",
      "DL (rigorous)": C.teal,
      "DL (CV)":       C.orange,
      // python_doubleml_pension labels — naive vs PLR vs IRM vs IIVM x 4 learners
      "Naive (mean diff)":  C.muted,
      "PLR-Lasso":          C.steel,
      "PLR-Random Forest":  C.steel,
      "PLR-Decision Tree":  C.steel,
      "PLR-XGBoost":        C.steel,
      "IRM-Lasso":          C.orange,
      "IRM-Random Forest":  C.orange,
      "IRM-Decision Tree":  C.orange,
      "IRM-XGBoost":        C.orange,
      "IIVM-Lasso":         C.teal,
      "IIVM-Random Forest": C.teal,
      "IIVM-Decision Tree": C.teal,
      "IIVM-XGBoost":       C.teal,
    };

    // Auto-detect whether the data is in dollars (large absolute values) and pick a tick formatter.
    function pickTickFormat(rows) {
      const maxAbs = d3.max(rows, d => Math.max(Math.abs(d.estimate || 0),
                                                 Math.abs(d.ci_lo || 0),
                                                 Math.abs(d.ci_hi || 0))) || 0;
      if (maxAbs > 100) {
        return d => d3.format("$,.0f")(d);
      }
      return d3.format(".2f");
    }
    function pickTooltipFormat(maxAbs) {
      if (maxAbs > 100) return v => d3.format("$,.0f")(v);
    ...
    function update(data, activeMethods, activeOutcomes) {
      // Infer outcomes/methods from data if no defaults match.
      ...
      const tickFmt = pickTickFormat(rows);
      const maxAbsRow = d3.max(rows, d => Math.max(Math.abs(d.estimate || 0),
                                                    Math.abs(d.ci_lo || 0),
                                                    Math.abs(d.ci_hi || 0))) || 0;
      const ttFmt = pickTooltipFormat(maxAbsRow);
        ...
        subset.forEach(d => {
          ...
          g.on("mousemove", function (ev) {
            ...
            tooltip.html(
              ...
              `<div><span class='tooltip-key'>α̂ =</span> <span class='tooltip-val'>${ttFmt(d.estimate)}</span></div>` +
              `<div><span class='tooltip-key'>SE =</span> <span class='tooltip-val'>${ttFmt(d.se)}</span></div>` +
              `<div><span class='tooltip-key'>95% CI =</span> <span class='tooltip-val'>[${ttFmt(d.ci_lo)}, ${ttFmt(d.ci_hi)}]</span></div>` +
              `<div><span class='tooltip-key'>covariates used =</span> <span class='tooltip-val'>${d.n_selected === null ? "—" : d.n_selected}</span></div>`
            )
            .classed("show", true)
