...
## One dataset, four estimators, very different answers
...
[Eligibility is set by the *employer*; participation is a *household choice*. That distinction drives all three models.]{.comment}
...
[The naive \$19,559 is not wrong arithmetic — it is the right gap answering the wrong question. DML's whole job is to subtract the second term.]{.comment}
...
[Regress $\tilde Y$ on $\tilde D$: the slope is $\hat\theta_0$. Both residuals are cleaned of confounding, so only the causal channel remains.]{.comment}
...
[Orthogonality kills *regularization* bias; cross-fitting kills *overfitting* bias. Together they let Lasso, forests, or XGBoost serve as nuisance learners with no harm to inference.]{.comment}
...
[The doubly-robust (AIPW) score: an outcome model corrected by inverse-propensity weighting. Consistent if *either* $g_0$ or $m_0$ is right — a safety net.]{.comment}
...
[A Wald-type ratio: the instrument's effect on savings, divided by its effect on participation.]{.comment}
...
[The LATE is the effect of participation on the marginal households a policy actually moves.]{.comment}
...
::: {.notes}
Steelman, don't strawman. DML's contribution is honest, efficient estimation given identification — not identification itself. Conditional exogeneity is untestable; the IV exclusion restriction is an assumption, not a result. The 1991 SIPP is also a single cross-section, and extreme asset outliers (−$502k to +$1.5M) pull the mean-based ATE. The method is a microscope, not a randomizer.
:::
