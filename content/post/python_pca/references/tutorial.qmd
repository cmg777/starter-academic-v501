...
subtitle: "Building a composite Health Index from Life Expectancy and Infant Mortality using manual PCA with simulated data for 50 countries, then verifying against scikit-learn"
author: "Carlos Mendez"
date: "2026-03-21"
jupyter: python-pca-tutorial
format:
  html:
    theme: cosmo
...
- **Example.** `infant_mort` is negatively coded — fewer infant deaths means better health. The raw correlation between `life_exp` and `infant_mort` is -0.9595. After multiplying `infant_mort` by -1, the correlation flips to +0.9595, and the two now point in the same direction.
- **Analogy.** Flipping the height-of-trash indicator to the negative before adding it to the cleanliness score.
...
- **Example.** In this post, raw `life_exp` lives in years (54.9-84.7) and raw `infant_mort` lives in deaths-per-1,000 (3.5-58.7). After z-scoring, both have mean 0.000000 and SD 1.000000 — directly comparable units.
- **Analogy.** Converting Celsius and Fahrenheit to the same unit before adding them.
...
- **Example.** With two standardized variables, the covariance matrix is 2×2 with 1.0000 on the diagonal and 0.9595 on the off-diagonal — the two indicators co-move strongly after polarity adjustment.
- **Analogy.** Which dance partners always move together — the covariance is how tightly they hold hands.
...
- **Example.** The eigen-decomposition of this post's 2×2 covariance matrix produces eigenvalues 1.9595 and 0.0405 with eigenvectors [0.7071, 0.7071] and [0.7071, -0.7071] — a perfect 45° decomposition.
- **Analogy.** Finding the axes a spinning top wants to rotate around — the spin "lives" along those axes.
...
- **Example.** In this post the first eigenvalue is 1.9595 — almost the entire 2.0 total variance. The first eigenvector [0.7071, 0.7071] equally weights both indicators. The second component (eigenvalue 0.0405) is essentially noise.
- **Analogy.** Eigenvalue = the strength of each spin axis; eigenvector = which way the axis points.

**6. Variance explained** $\lambda_k / \sum \lambda$.
The fraction of total variance captured by component $k$. Sums to 1 across all components.

- **Example.** PC1 explains 97.97% of variance ($1.9595 / 2.0000$); PC2 explains just 2.03%. PC1 alone is enough to summarize health.
- **Analogy.** "How much of the spin lives along this axis."

**7. Component score (PC1)** $s_i = Z_i v_1$.
The projection of country $i$'s standardized data onto the first eigenvector. Each country's value on the new latent axis.

- **Example.** In this post the PC1 scores range from -2.3892 (worst-performing country) to +2.3734 (best-performing). The score is a single number that captures the country's overall health.
- **Analogy.** The team's combined-skills overall rating, computed by mixing all the individual stats.

**8. Min-max normalization** $(s - s_{\min}) / (s_{\max} - s_{\min})$.
Rescale scores into [0, 1]. Useful for indices that policymakers expect on a 0-to-100 (or 0-to-1) scale.

- **Example.** The post rescales the [-2.3892, +2.3734] PC1 scores to a Health Index on [0, 1]. The mean Health Index is 0.5017 — close to 0.5, as expected by symmetry.
- **Analogy.** Stretching the scoreboard so the worst is 0 and the best is 1.
...
#| fig-cap: "Raw health indicators: Life Expectancy vs. Infant Mortality for 50 simulated countries. The annotated correlation is strongly negative."
...
```{python}
#| label: fig-standardized-eigenvectors
#| fig-cap: "Standardized data with PC1 and PC2 eigenvector arrows overlaid. PC1 (orange) points along the diagonal direction of maximum spread; PC2 (teal) is perpendicular and captures only residual noise."
...
#| fig-cap: "Bar chart showing PC1 captures 98.0% and PC2 captures 2.0% of total variance."
...
```{python}
#| label: fig-pc1-scores
#| fig-cap: "Horizontal bar chart of 50 countries ranked by PC1 score, with positive scores in teal and negative in warm orange."
...
```{python}
#| label: fig-health-index
#| fig-cap: "Health Index bar chart for 50 countries with gradient coloring from orange (low health) to teal (high health)."
...
```{python}
#| label: fig-sklearn-comparison
#| fig-cap: "Scatter plot showing perfect agreement between manual and sklearn PCA scores. All 50 points fall on the 45-degree line."
...
The maximum absolute difference between manual and sklearn PC1 scores is essentially machine-precision zero. The correlation is 1.000000, confirming perfect agreement. All 50 points fall exactly on the dashed 45-degree line. This validates that our step-by-step manual implementation produces identical results to the optimized library.
...
- Companion script: [`script.py`](https://raw.githubusercontent.com/cmg777/starter-academic-v501/master/content/post/python_pca/script.py)
- Published post: <https://carlos-mendez.org/post/python_pca/>
