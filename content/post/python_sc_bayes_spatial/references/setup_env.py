...
into it, registers a named Jupyter kernel (`sc-bayes-spatial-tutorial`) that points at
...
# Version SPECIFIERS rather than exact pins for the scientific stack.
# setup_env.py may fall back to a Python as old as 3.10, which cannot install
# the newest numpy/pandas, so pinning those exactly would make the bundle
# unbuildable on a perfectly valid interpreter. The post itself was produced
# with numpy 2.3.5, pandas 3.0.1, scipy 1.17.1 and matplotlib 3.10.8 on
# Python 3.13.
#
# The two packages whose numbers this tutorial reproduces ARE pinned exactly.
PINNED: dict[str, str] = {
    "numpy":      ">=1.26,<3",
    "pandas":     ">=2.0",
    "scipy":      ">=1.11",
    "matplotlib": ">=3.8",
    # scspill has a clean PyPI release and a matching git tag; 0.2.1 is the
    # version every number in the post was produced under. The `[numba]` extra
    # makes the samplers about five times faster and returns bit-identical
    # results, but numba lags each new CPython release by months -- so if the
    # extra cannot be installed, plain scspill is a correct fallback and the
    # script says so at runtime rather than failing.
    "scspill":    "[numba]==0.2.1",
    # NOT "==1.0.0". The PyPI release numbered 1.0.0 is BEHIND git main at the
    # same version number. Pin the commit the post was verified against.
    # The [bayes] extra pulls numpyro, which BFSC, MVBBSC, BPSCS and SPOTSYNTH
    # all import -- without it four rows of the section 13 table fail.
    "mlsynth":    "[bayes] @ git+https://github.com/jgreathouse9/mlsynth.git"
                  "@15f168bb90487098a7324be00b6663fcab0139ef",
    "jupyter":    ">=1.0",
    "ipykernel":  ">=6.29",
}

KERNEL_NAME = "sc-bayes-spatial-tutorial"
KERNEL_DISPLAY = "Bayesian Spatial SC Tutorial"
...
def ensure_packages_in_venv() -> None:
    """Install any package that is missing, letting pip resolve the specifier.

    Packages already present are left alone: re-resolving them on every render
    would turn an idempotent no-op into a network round-trip.
    """
    py = venv_python(VENV_DIR)
    to_install = [f"{pkg}{spec}" for pkg, spec in PINNED.items()
                  if installed_version(py, pkg) is None]
    if not to_install:
        return
    ...
    # The only optional piece is scspill's `[numba]` extra. numba lags each new
    # CPython release by months, so on a very new interpreter there may be no
    # wheel and the whole batch fails. scspill works fine without it -- the
    # samplers fall back to numpy, about five times slower, same answers -- so
    # retry once with the extra stripped rather than leaving the reader stuck.
    relaxed = [s.replace("scspill[numba]", "scspill") for s in to_install]
    if relaxed == to_install:
        raise
    print("  [WARN] install failed; retrying without scspill's optional [numba] extra.")
    print("         The tutorial will run on the numpy backend: same results,")
    print("         roughly five times slower. Section 5 of the post explains why.")
    subprocess.check_call([*pip, *relaxed])
...
def preflight() -> None:
    ...
    if version_unsupported:
        ...
        print(
            ...
            f"and Python <=3.9 is below mlsynth's minimum.\n\n"
...
def main() -> None:
    preflight()
    print("Setting up the Bayesian spatial SC tutorial environment...")
    ensure_venv()
    ensure_pip_in_venv()
