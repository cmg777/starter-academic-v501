...
[We never trust a number on real data that the same method could not first recover on simulated ground truth.]{.comment}
...
[Formula mini-language: `outcome ~ treatment | covariates`.]{.comment}
...
[The first term is the plain SCM gap; the second is the Ridge correction $\hat m_t$. Perfect pre-fit → correction vanishes → ASCM $=$ SCM.]{.comment}
...
::: {.notes}
Twelve single_augsynth fits, each member vs the 24 donors. Most members run above their synthetic counterfactuals after 1999. Greece and Portugal converge to or fall below theirs after the crisis (Greece +8.0% → −12.4%, Portugal +6.5% → −14.3%) — matching the paper's negative post-crisis contributions. Averages hide dynamics; the per-unit spread is the honest multi-country report.
:::
...
[We do not reproduce the paper's numbers exactly — and should not. Same signs, same ranking, same dynamic story is the right bar.]{.comment}
