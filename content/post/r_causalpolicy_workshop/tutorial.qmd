...
We use `pacman::p_load()` to install (if needed) and load every package in a single line. The chunk below is safe to re-run on a fresh R installation: if `pacman` itself is missing, it is installed from CRAN first. A global `set.seed(42)` fixes the random-forest imputation and the CausalImpact MCMC sampler so the numbers in this document are bit-reproducible.

```{r}
#| label: setup-packages

# pacman is a tiny meta-package that auto-installs missing CRAN packages.
# Bootstrap it first so the rest of the tutorial can run on a fresh machine.
if (!requireNamespace("pacman", quietly = TRUE)) {
  install.packages("pacman", repos = "https://cloud.r-project.org")
}

# p_load() installs (if missing) and attaches every package this tutorial
# needs --- one line instead of one library() call per package.
pacman::p_load(
  ...
  forcats       # factor reordering (used in §10.2 V-matrix chart)
)
