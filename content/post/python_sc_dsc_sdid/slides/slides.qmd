...
::: {.notes}
Four acts. Act I is the problem. Act II climbs the ladder — this is the longest stretch and the most code. Act III is where the deck earns its title: the defaults. Act IV asks which stage to choose and how confident we are allowed to be about the answer.
:::

# One Config, Six Stages {.divider background-color="#6a9bcc"}
...
[The stages differ only in how $\omega$ and $\lambda$ are chosen.]{.takeaway .fragment}
...
[Learn these seven and you can read any estimator in the library — no docs needed.]{.takeaway .fragment}
...
[The synthetic UK: a fifth Hungary, a fifth the US, a fifth Japan — plus Canada and Norway.]{.takeaway .fragment}

::: {.notes}
The two smaller shares are a sixth Canada and an eighth Norway; fourteen of the twenty-three donors get nothing at all.
...
[DSC treats all 86 pre-treatment quarters alike. SDID does not.]{.takeaway .fragment}
...
[Before: $+0.0021$. After: $-0.0281$. A falsification test for one extra line of code.]{.takeaway .fragment}
...
## Stages 4 and 5 — buy the trade-off, or drop the constraint {.smaller}
...
::: {.notes}
Five estimators built on quite different principles are picking essentially the same five countries. That is reassuring in a way the point estimates alone are not: the disagreement between stages is not a disagreement about which countries resemble the UK.

MASC spreads a small uniform-looking 0.0158 across six otherwise-zero donors — Austria, Belgium, France, the Netherlands, Spain and Sweden. That is the matching component showing through.
:::
...
::: {.notes}
The published column is the R edition of this post, using synthdid, Synth, masc and augsynth. Three of the six stages — DiD, MASC and ASCM — agree to two decimals. DSC lands a hundredth above only because 2.9887 sits on a rounding boundary.
...
![Every stage's estimated UK GDP shortfall at 2018Q4 and 2019Q4, with the three SDID flavours shown separately; the dashed line is the published 2.4%.](../python_sc_dsc_sdid_11_att_dotplot.png)

[**2.73–3.04%** at end-2018 and **3.83–4.19%** a year later — every stage above the 2.4% previously published for this same dataset.]{.takeaway .fragment}
...
[Three languages, two camps — split by optimiser, not by author. Land 0.02 away? Suspect the solver first.]{.takeaway .fragment}
...
# Which Stage? {.divider background-color="#00d4c8"}
...
::: {.notes}
Everything the earlier acts taught about defaults now pays off in wall-clock time. Twenty dates times seven estimators, refit from scratch each pass, is 140 fits. The four settings that make it tractable are `vce="noinference"` on SDID, `method="MSCa"` and `inference=False` on TSSC, and an explicit `m_grid` on MASC. The post puts the cost of leaving those settings alone plainly: a two-minute loop becomes a two-hour one.

This is the source paper's answer to "which stage", and it is a good one — but read the next slide before accepting the ranking it produces.
:::
...
[Response.]{.rebuttal} Four p-values span $0.006$ to $0.042$; `scpi`'s interval stops short of zero. Five routes, all rejecting at 5%. But the floor is real, and no estimator changes it.
...
[UK GDP shortfall at end-2018, at every fitted stage of the ladder · 3.8–4.2% a year later · against 2.4% previously published]{.bignum-label}

::: {.notes}
That is the answer, and the range is the answer — not a rounding of some sharper number we could have reported instead. Every stage of the ladder puts the cost above the previously published 2.4%, and the reason is not exotic: the earlier figure came from a specification that matched on covariates, and on this panel covariates make the counterfactual worse.
:::
...
## Everything here ships with the post {.smaller}
