...
into it, registers a named Jupyter kernel (`sc-dsc-sdid-tutorial`) that points at
...
# Version SPECIFIERS rather than exact pins. setup_env.py may fall back to a
# Python as old as 3.10, which cannot install the newest numpy/pandas, so
# pinning those exactly makes the bundle unbuildable on a perfectly valid
# interpreter. Only mlsynth is pinned exactly -- it is the package whose
# numbers this tutorial reproduces. The post itself was produced with
# numpy 2.3.5, pandas 3.0.1 and matplotlib 3.10.8 on Python 3.13.
PINNED: dict[str, str] = {
    "numpy":      ">=1.26,<3",
    "pandas":     ">=2.0",
    "matplotlib": ">=3.7",
    # NOT "==1.0.0". The PyPI release numbered 1.0.0 is BEHIND git main at the
    # same version number -- it lacks VanillaSCConfig.w_constr, which this
    # tutorial uses. Pin the commit the post was verified against.
    "mlsynth":    " @ git+https://github.com/jgreathouse9/mlsynth.git"
                  "@15f168bb90487098a7324be00b6663fcab0139ef",
    "jupyter":    ">=1.0",
    "ipykernel":  ">=6.29",
}

KERNEL_NAME = "sc-dsc-sdid-tutorial"
KERNEL_DISPLAY = "SC-DSC-SDID Tutorial"
...
def main() -> None:
    preflight()
    print("Setting up the SC-DSC-SDID tutorial environment...")
    ensure_venv()
    ensure_pip_in_venv()
