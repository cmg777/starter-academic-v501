...
- icon: file-pdf
  icon_pack: fas
  name: "AI Slides (PDF)"
  url: https://carlos-mendez.org/post/python_ml_random_forest/slides/Mapping_Bolivian_Development_with_Satellites.pdf
- icon: laptop-code
  icon_pack: fas
  name: "Web app"
...
Satellite imagery is increasingly used as a low-cost proxy for socioeconomic conditions in places where survey data are sparse, raising the question of how much development-related signal it actually contains. This tutorial sets out to predict Bolivia's Municipal Sustainable Development Index (IMDS), a composite score on a 0—100 scale, from satellite image embeddings, and to introduce machine-learning best practices along the way. The data come from the DS4Bolivia repository and cover all 339 Bolivian municipalities with complete coverage and no missing values, pairing IMDS scores (mean 51.05, standard deviation 6.77, ranging 35.70 to 80.20) with 64-dimensional feature vectors extracted from 2017 satellite imagery. A Random Forest regressor is trained on an 80/20 train/test split (271 training and 68 test municipalities), evaluated with 5-fold cross-validation, and tuned via randomized search over tree count, depth, and feature subsampling. The baseline model reaches a test R² of 0.2307 (RMSE 6.52, MAE 4.68), and tuning to 500 trees with max_depth=30 yields an essentially identical test R² of 0.2297 (RMSE 6.52, MAE 4.72), explaining about 23% of IMDS variation, with permutation importance highlighting embedding dimensions A59, A42, and A26 and partial dependence plots revealing non-linear threshold effects. These results imply that satellite embeddings capture real but limited development signal—predictions regress toward the 47—55 range and miss extreme cases—so combining them with administrative or survey data is the natural next step.
...
<div class="podcast-inner">
  <audio id="podAudio" preload="none" src="https://files.catbox.moe/uan9vt.m4a"></audio>
  ...
  <div class="podcast-controls-row">
    ...
    <div class="podcast-extras">
      ...
      <a class="podcast-download-btn" href="https://files.catbox.moe/uan9vt.m4a" target="_blank" rel="noopener" title="Stream">
