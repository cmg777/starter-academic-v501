...
[Bootstrap rows + random feature subsets = trees that make *different* mistakes; averaging cancels the noise.]{.takeaway .fragment}
...
[A single 80/20 split is a lottery: across 200 random seeds the test R-squared ranges from −0.09 to 0.46 (Appendix A).]{.takeaway .fragment}
...
[Fold 3 scores **−0.03** — worse than guessing the average. One lucky split could have shown you only the 0.45.]{.takeaway .fragment}
...
[Two very different methods crowning the same feature is evidence A30 is real signal, not a counting artifact.]{.takeaway .fragment}
...
[The methods rank as theory predicts — Optuna ≥ random ≥ grid — but every gain is smaller than the 0.17 fold-to-fold noise.]{.takeaway .fragment}
