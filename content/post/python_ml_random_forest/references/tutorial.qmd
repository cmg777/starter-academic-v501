...
Reuse the same `kf` and the same `cross_validate` / `cross_val_predict` calls — only the model and feature set change.
...
The linear model explains only ~6% of IMDS variation (pooled OOF R² ≈ 0.06) — low, but the *workflow* is the lesson, not the score. The same pipeline, with more features or a flexible model, does better.

### C.4 Evaluation plots

```{python}
#| label: fig-lr-actual-vs-predicted
#| fig-cap: "Linear regression: out-of-fold predicted vs actual IMDS, colored by fold."
fig, ax = plt.subplots(figsize=(7.2, 7.2))
for k in range(1, N_FOLDS + 1):
    m = fold_id == k
...
#| fig-cap: "Linear regression: out-of-fold residuals, colored by fold."
fig, ax = plt.subplots(figsize=(8, 5))
for k in range(1, N_FOLDS + 1):
    m = fold_id == k
...
With so little signal, predictions barely spread — almost every town is pulled to the mean of ~51, and the residuals stay large.
...
```{python}
#| label: fig-lr-importance
#| fig-cap: "Linear regression: standardized (signed) coefficients."
...
A00 dominates; A01 is essentially zero. This is the linear analogue of permutation importance — free, signed, straight from the equation.

### C.6 Partial dependence

```{python}
#| label: fig-lr-partial-dependence
#| fig-cap: "Linear regression: partial dependence is a straight line for every feature."
...
Every panel is a **straight line** — a linear model's effect is constant, the slope being the coefficient. Contrast the forest's bending, plateauing curves (Section 11): that is exactly what a flexible model buys you.
