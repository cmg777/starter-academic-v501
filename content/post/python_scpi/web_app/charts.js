...
(function () {
  ...
  // ------------------------------------------------------------------
  // SCPI: Donor pool animation (Tab 1).
  //   Animates the construction of "synthetic West Germany" from donor
  //   countries: each donor's bar shows its weight; the synthetic line
  //   blends pulse-by-pulse from the donor weighted-average.
  // ------------------------------------------------------------------
  function donor_pool_animation(container, weights) {
    const W = 720, H = 320;
    const margin = { top: 28, right: 24, bottom: 40, left: 100 };
    const w = W - margin.left - margin.right;
    const h = H - margin.top - margin.bottom;
    const svg = ensureSVG(container, W, H);
    const g = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);

    const data = weights.slice().sort((a, b) => a.weight - b.weight);
    const xMax = d3.max(data, d => d.weight) || 0.35;
    const x = d3.scaleLinear().domain([0, xMax * 1.15]).range([0, w]);
    ...
    g.append("text")
      ...
      .attr("text-anchor", "middle").attr("fill", C.text).attr("font-size", 12)
      .text("Donor weight (sums to 1.000)");
    ...
    const bars = g.selectAll(".dbar").data(data).enter().append("rect")
      ...
      .attr("x", 0)
      .attr("width", 0)
      .attr("fill", (d, i) => i === data.length - 1 ? C.orange : C.steel)
      .attr("opacity", 0.9);

    const labels = g.selectAll(".dlbl").data(data).enter().append("text")
      .attr("class", "dlbl")
      .attr("x", 4)
    ...
    function step(ts) {
      ...
      const elapsed = (ts - t0) / 1000;
      const cycle = (Math.sin(elapsed * 0.6) + 1) / 2; // [0, 1]
      bars.attr("width", d => x(d.weight) * cycle);
      labels.attr("x", d => x(d.weight) * cycle + 6)
        .text(d => cycle > 0.4 ? d.weight.toFixed(3) : "");
      requestAnimationFrame(step);
    }
    requestAnimationFrame(step);
  }
  ...
  // ------------------------------------------------------------------
  // SCPI: Actual vs Synthetic trajectory + Prediction Interval band (Tab 2).
  //   Plots actual West Germany, synthetic counterfactual, and PI band
  //   across post-treatment years. Vertical reunification line at 1990.
  ...
  function scpi_trajectory(container) {
    ...
    const band = g.append("path").attr("fill", C.steel).attr("opacity", 0.18);
    const synthLine = g.append("path").attr("fill", "none").attr("stroke", C.steel)
      .attr("stroke-width", 2.2).attr("stroke-dasharray", "5 4");
    const actualLine = g.append("path").attr("fill", "none").attr("stroke", C.orange)
    ...
    function update(data) {
      ...
      const mult = data.piMult || 1;
      const x = d3.scaleLinear().domain(d3.extent(tr, d => d.year)).range([0, w]);
      const allVals = tr.flatMap(d => {
        // Scale PI width relative to synthetic.
        const half = (d.pi_hi - d.pi_lo) / 2 * mult;
        return [d.actual, d.synthetic - half, d.synthetic + half];
      });
      ...
      const area = d3.area()
        .x(d => x(d.year))
        .y0(d => y(d.synthetic - (d.pi_hi - d.pi_lo) / 2 * mult))
        .y1(d => y(d.synthetic + (d.pi_hi - d.pi_lo) / 2 * mult))
        .curve(d3.curveMonotoneX);
      band.attr("d", area(tr));

      const lineA = d3.line().x(d => x(d.year)).y(d => y(d.actual)).curve(d3.curveMonotoneX);
      const lineS = d3.line().x(d => x(d.year)).y(d => y(d.synthetic)).curve(d3.curveMonotoneX);
      actualLine.attr("d", lineA(tr));
      synthLine.attr("d", lineS(tr));

      // Color dots by whether actual is outside the PI.
      dotsG.selectAll(".pi-dot").remove();
      tr.forEach(d => {
        const half = (d.pi_hi - d.pi_lo) / 2 * mult;
        const lo = d.synthetic - half;
        const hi = d.synthetic + half;
        const outside = d.actual < lo || d.actual > hi;
        dotsG.append("circle")
          .attr("class", "pi-dot")
          .attr("cx", x(d.year)).attr("cy", y(d.actual))
          .attr("r", outside ? 5 : 3.5)
          .attr("fill", outside ? C.orange : C.muted)
          .attr("stroke", outside ? "#fff" : "none")
          .attr("stroke-width", outside ? 1 : 0);
      });
    }
    return { update };
  }
  ...
  // ------------------------------------------------------------------
  // SCPI: Gap-over-time chart (Tab 3).
  //   Plots actual minus synthetic gap and a shaded PI half-width.
  //   data: same trajectory as scpi_trajectory; also takes piMult.
  // ------------------------------------------------------------------
  function scpi_gap(container) {
    const W = 720, H = 280;
    ...
    g.append("text")
      .attr("transform", `rotate(-90) translate(${-h / 2},${-44})`)
      .attr("text-anchor", "middle").attr("fill", C.text).attr("font-size", 12)
      .text("Gap = Actual − Synthetic (thousand USD)");

    const band = g.append("path").attr("fill", C.orange).attr("opacity", 0.18);
    const zeroLine = g.append("line").attr("stroke", C.faint).attr("stroke-dasharray", "3 4");
    const gapLine = g.append("path").attr("fill", "none").attr("stroke", C.orange).attr("stroke-width", 2.5);
    const xAxisG = g.append("g").attr("transform", `translate(0,${h})`);
    const yAxisG = g.append("g");
    const dotsG = g.append("g");
    ...
    function update(data) {
      ...
      const rows = tr.map(d => {
        ...
        return {
          year: d.year,
          gap: d.actual - d.synthetic,
          lo: -half,
          hi: half,
        };
      });
      const x = d3.scaleLinear().domain(d3.extent(rows, d => d.year)).range([0, w]);
      const all = rows.flatMap(d => [d.gap, d.lo, d.hi]);
      const yExt = d3.extent(all);
      const span = yExt[1] - yExt[0];
      ...
      const area = d3.area().x(d => x(d.year))
        .y0(d => y(d.lo)).y1(d => y(d.hi)).curve(d3.curveMonotoneX);
      band.attr("d", area(rows));
      zeroLine.attr("x1", 0).attr("x2", w).attr("y1", y(0)).attr("y2", y(0));
      const line = d3.line().x(d => x(d.year)).y(d => y(d.gap)).curve(d3.curveMonotoneX);
      gapLine.attr("d", line(rows));

      dotsG.selectAll(".gap-dot").remove();
      rows.forEach(d => {
        const outside = d.gap < d.lo || d.gap > d.hi;
        dotsG.append("circle")
          .attr("class", "gap-dot")
          .attr("cx", x(d.year)).attr("cy", y(d.gap))
          .attr("r", outside ? 5 : 3.5)
          .attr("fill", outside ? C.orange : C.muted)
          .attr("stroke", outside ? "#fff" : "none")
          .attr("stroke-width", outside ? 1 : 0);
  ...
  // ------------------------------------------------------------------
  // SCPI: Method comparison forest plot (Tab 4).
  //   Horizontal CIs by (method × outcome). Same skeleton as forest_plot but
  //   uses SCPI's method names (Simplex / Lasso / Ridge / OLS) and outcomes
  //   ("Gap 2003" / "Avg gap").
  // ------------------------------------------------------------------
  function scpi_forest_plot(container) {
    const W = 880;
    const margin = { top: 28, right: 24, bottom: 36, left: 130 };
    const facetGap = 24;
    const svg = d3.select(container).html("").append("svg")
    ...
    function update(data, activeMethods, activeOutcomes) {
      ...
      svg.attr("viewBox", `0 0 ${W} ${totalH}`);
      svg.selectAll("g.facet").remove();
      svg.selectAll("text.facet-label").remove();
      ...
      outcomes.forEach((outcome, oi) => {
        ...
        const xMin = Math.min(0, ext[0] || 0);
        const xMax = Math.max(0, ext[1] || 0);
        const pad = Math.max(0.2, (xMax - xMin) * 0.08);
        const x = d3.scaleLinear().domain([xMin - pad, xMax + pad]).range([0, facetW]);
        const y = d3.scaleBand().domain(methods).range([0, facetH]).padding(0.35);
        ...
        subset.forEach(d => {
          ...
          grp.on("mousemove", function (ev) {
            const rect = container.getBoundingClientRect();
            tooltip.html(
              `<div><strong style="color:${colorMap[d.method]}">${d.method}</strong> — ${d.outcome}</div>` +
              `<div><span class='tooltip-key'>gap =</span> <span class='tooltip-val'>${d.estimate.toFixed(3)}</span></div>` +
              `<div><span class='tooltip-key'>SE =</span> <span class='tooltip-val'>${d.se.toFixed(3)}</span></div>` +
              `<div><span class='tooltip-key'>95% CI =</span> <span class='tooltip-val'>[${d.ci_lo.toFixed(3)}, ${d.ci_hi.toFixed(3)}]</span></div>` +
              `<div><span class='tooltip-key'>donors used =</span> <span class='tooltip-val'>${d.n_selected}</span></div>`
            )
            .classed("show", true)
            .style("left", (ev.clientX - rect.left + 12) + "px")
            .style("top",  (ev.clientY - rect.top  + 12) + "px");
  ...
  // ------------------------------------------------------------------
  // SCPI: Method comparison bar chart — pre-RMSE vs gap (Tab 4).
  //   Two-panel bars: left = pre-treatment RMSE (lower is better fit);
  //   right = absolute gap in 2003.
  // ------------------------------------------------------------------
  function scpi_method_bars(container) {
    const W = 880;
    const margin = { top: 24, right: 20, bottom: 50, left: 60 };
    const svg = d3.select(container).html("").append("svg")
      .attr("viewBox", `0 0 ${W} 260`)
      .attr("preserveAspectRatio", "xMidYMid meet");

    function update(data) {
      const methods = ["Simplex", "Lasso", "Ridge", "OLS"];
      const subset = data.filter(d => methods.includes(d.method));
      const nFacets = 2;
      const facetGap = 40;
      const facetW = (W - margin.left - margin.right - (nFacets - 1) * facetGap) / nFacets;
      const facetH = 160;
      ...
      const panels = [
        { key: "pre_rmse", title: "Pre-treatment RMSE  (lower = better fit)", color: C.steel },
        { key: "gap_2003", title: "Gap in 2003 (thousand USD)", color: C.orange, abs: true },
      ];
      panels.forEach((p, pi) => {
        const facet = svg.append("g")
          .attr("class", "facet")
          .attr("transform", `translate(${margin.left + pi * (facetW + facetGap)},${margin.top})`);
        const vals = subset.map(d => p.abs ? Math.abs(d[p.key]) : d[p.key]);
        ...
        facet.append("text").attr("x", facetW / 2).attr("y", -8)
          .attr("text-anchor", "middle").attr("fill", C.text).attr("font-size", 12)
          .attr("font-weight", 600).text(p.title);
        facet.append("g").attr("transform", `translate(0,${facetH})`)
          .call(d3.axisBottom(x).tickSize(0))
        ...
        facet.selectAll(".domain, .tick line").attr("stroke", C.muted);
        subset.forEach(d => {
          const v = p.abs ? Math.abs(d[p.key]) : d[p.key];
          facet.append("rect")
            .attr("x", x(d.method)).attr("y", y(v))
            .attr("width", x.bandwidth()).attr("height", facetH - y(v))
            .attr("fill", p.color).attr("opacity", 0.85);
          facet.append("text")
            .attr("x", x(d.method) + x.bandwidth() / 2)
            .attr("y", y(v) - 4)
            .attr("text-anchor", "middle")
            .attr("fill", C.text).attr("font-size", 11).attr("font-weight", 600)
            .text((p.abs ? "−" : "") + v.toFixed(p.key === "pre_rmse" ? 3 : 2));
        });
      });
    }
    return { update };
