...
subtitle: "Synthetic control with prediction intervals quantifies uncertainty in Germany's reunification GDP impact using the scpi package"
author: "Carlos Mendez"
date: "2026-03-22"
jupyter: python_scpi-tutorial
format:
  html:
    theme: cosmo
...
The post leans on a small vocabulary repeatedly. Each concept below has three parts: a **definition**, an **example**, and an **analogy**.

**1. Synthetic control method** $\hat{Y}_T = \sum_j w_j Y_{j}$.
...
- *Example.* This tutorial builds a synthetic West Germany from 16 OECD donor countries using only their pre-1990 GDP per capita. After 1990, the synthetic continues following the donor weighted-average — that is the counterfactual.
- *Analogy.* A custom-built body double for an actor. Same height, same hair, same gestures *before* the dangerous scene.
...
- *Example.* The donor pool has 16 OECD countries (Austria, USA, France, Japan, etc.). Reunification did not affect them, so they can plausibly proxy what West Germany would have looked like.
- *Analogy.* The casting candidates for the body double. The bigger the casting list, the better the match.

**3. Weight constraints (simplex)** $w_j \ge 0$, $\sum w_j = 1$.
...
- *Example.* The simplex selects 6 of 16 donors with non-zero weight. Austria's weight is `0.291` and the USA's is `0.273`. The remaining 10 donors get weight 0.
- *Analogy.* "No negative casting" — every actor weighs in non-negatively, and the casting fractions add to 1.

**4. Pre-treatment fit (RMSE)** $\sqrt{\frac{1}{T_0}\sum_t (Y_t - \hat Y_t)^2}$.
...
- *Example.* The simplex synthetic has pre-treatment `RMSE = 0.072` — about 0.6% of West Germany's pre-1990 GDP. The body double looks essentially identical before the scene.
- *Analogy.* How well the body double mimics the actor *before* the scene starts.

**5. Treatment gap** $Y_T - \hat Y_T$.
...
- *Example.* In 1991 the gap is `+0.502` (West Germany slightly outpaces synthetic immediately post-reunification). By 2003 the gap is `-3.465` thousand USD — a roughly 11% GDP reduction relative to counterfactual. Average gap over 1991-2003 is `-1.668`.
- *Analogy.* The costume difference *after* the scene — what the actor wears that the body double does not.

**6. Prediction interval** $[\hat Y_T - q, \hat Y_T + q]$.
...
- *Example.* The average 95% PI width is `2.842` thousand USD. By 2003, the *actual* GDP falls below the 99% PI in 7 of the 13 post-treatment years — strong evidence that the gap is real and not noise.
- *Analogy.* The range the body double could plausibly stand in.

**7. In-sample vs out-of-sample uncertainty** $\sigma^2_{\mathrm{in}}$, $\sigma^2_{\mathrm{out}}$.
...
- *Example.* Even though the simplex achieves pre-treatment `RMSE = 0.072` (very low in-sample uncertainty), the out-of-sample uncertainty grows over time and dominates the 2003 PI width.
- *Analogy.* Stage-rehearsal noise (predictable) vs opening-night noise (unpredictable). The longer the show, the more opening-night noise accumulates.
...
- *Example.* The tutorial compares simplex, ridge, lasso, and OLS weighting and finds the qualitative picture (post-1990 negative gap) is robust. Sensitivity also varies the confidence level from 90% to 99% — significance survives.
- *Analogy.* Checking the body double scene with two or three different doubles and seeing the same effect.
...
```{python}
#| label: fig-gdp-trajectories
#| fig-cap: "GDP trajectories of 17 countries from 1960 to 2003, with West Germany highlighted and a vertical line at 1990 marking reunification."
...
```{python}
#| label: fig-actual-vs-synthetic
#| fig-cap: "Actual and synthetic West Germany GDP from 1960 to 2003, showing close pre-treatment tracking and post-1990 divergence."
...
```{python}
#| label: fig-weights
#| fig-cap: "Horizontal bar chart of synthetic control weights showing Austria (0.291), USA (0.273), Italy (0.191), Netherlands (0.133), Switzerland (0.081), and France (0.030)."

fig, ax = plt.subplots(figsize=(8, 5))
countries_w = [idx[1] if isinstance(idx, tuple) and len(idx) > 1 else idx for idx in w_df.index]
weights = w_df['weight'].values

bars = ax.barh(range(len(countries_w)), weights, color='#6a9bcc', height=0.6)

# Highlight the largest weight
max_idx = int(np.argmax(weights))
bars[max_idx].set_color('#d97757')

ax.set_yticks(range(len(countries_w)))
ax.set_yticklabels(countries_w)
ax.set_xlabel('Weight')
ax.set_title('Synthetic Control Weights (Simplex)')
...
```{python}
#| label: fig-treatment-gap
#| fig-cap: "Bar chart showing the year-by-year treatment effect gap from 1991 to 2003, growing increasingly negative over time."

fig, ax = plt.subplots(figsize=(10, 5))
colors = ['#d97757' if g < 0 else '#00d4c8' for g in gap_post]
ax.bar(period_post, gap_post, color=colors, width=0.7)
ax.axhline(y=0, color='#141413', linewidth=0.8, alpha=0.5)
ax.set_xlabel('Year')
ax.set_ylabel('Gap (Actual - Synthetic, thousand USD)')
ax.set_title('Estimated Treatment Effect of Reunification')
plt.show()
```
...
#| fig-cap: "Synthetic control with prediction interval bands showing actual West Germany GDP falling below the lower bound after the mid-1990s."
...
#| fig-cap: "Four-panel comparison showing actual vs. synthetic GDP under simplex, lasso, ridge, and OLS weight constraints."
...
for ax, (name, est) in zip(axes.flatten(), methods.items()):
    ...
    ax.plot(t_all, np.concatenate([y_pre_a, y_post_a]),
            color='#d97757', linewidth=1.8, label='Actual')
    ax.plot(t_all, np.concatenate([y_pre_f, y_post_f]),
            color=color, linewidth=1.8, linestyle='--',
            label=f'Synthetic ({name})')
    ax.axvline(x=1990, color='#00d4c8', linestyle=':', linewidth=1, alpha=0.6)
...
fig.suptitle('Comparing Weight Constraint Methods', y=1.02)
plt.tight_layout()
plt.show()
```
...
- Companion script: [`script.py`](https://raw.githubusercontent.com/cmg777/starter-academic-v501/master/content/post/python_scpi/script.py)
- Published post: <https://carlos-mendez.org/post/python_scpi/>
