...
[$Y_{1T}(1)$ is observed; $Y_{1T}(0)$ — West Germany *without* reunification — is never observed, so we estimate it.]{.takeaway .fragment}
...
[Non-negative weights that sum to one make synthetic West Germany a *convex combination* — it never extrapolates beyond the real donor data.]{.takeaway .fragment}
...
[`cointegrated_data=True`: GDP series share a common upward drift, so the estimator matches the trend, not a fixed level.]{.takeaway .fragment}
...
[Average gap 1991–2003: −$1,668 per capita — a substantial, *growing* cost.]{.takeaway .fragment}
...
[Every constraint agrees: reunification lowered GDP. Only the magnitude shifts.]{.takeaway .fragment}
