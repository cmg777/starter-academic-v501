...
subtitle: "Applying MGWR to reveal how economic catching-up varies across Indonesia's 514 districts, with each variable operating at its own spatial scale"
author: "Carlos Mendez"
date: "2026-03-22"
jupyter: python_mgwr-tutorial
format:
  html:
    theme: cosmo
...
```{python}
#| label: fig-map-xy
#| fig-cap: "Two-panel choropleth map of Indonesia showing log GDP per capita in 2010 and GDP growth rate 2010-2018."
...
```{python}
#| label: fig-scatter-global
#| fig-cap: "Scatter plot of log GDP per capita 2010 vs growth rate with OLS regression line."
...
```{python}
#| label: fig-mgwr-intercept
#| fig-cap: "MGWR intercept map across Indonesia's 514 districts."
...
```{python}
#| label: fig-mgwr-slope
#| fig-cap: "MGWR convergence coefficient map across Indonesia."
...
```{python}
#| label: fig-mgwr-significance
#| fig-cap: "Significance map showing districts with statistically significant catching-up."
...
- **Next step:** Extend the model with additional covariates (education, investment, fiscal transfers) to disentangle the sources of spatial heterogeneity. Apply MGWR to panel data with multiple time periods. Compare MGWR results with the spatial clusters identified in the [ESDA tutorial](https://carlos-mendez.org/post/python_esda2/) to see whether convergence hotspots align with LISA clusters.
...
- Companion script: [`script.py`](https://raw.githubusercontent.com/cmg777/starter-academic-v501/master/content/post/python_mgwr/script.py)
- Published post: <https://carlos-mendez.org/post/python_mgwr/>
