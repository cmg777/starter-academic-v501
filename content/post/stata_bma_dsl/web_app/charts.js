...
// ============================================================
// Widget-specific extension for stata_bma_dsl: PIP chart.
// Horizontal bar chart of Posterior Inclusion Probabilities,
// colour-coded by ground truth (true predictor vs noise).
// ============================================================
(function () {
  "use strict";
  ...
  function pip_chart(container) {
    ...
    function update(rows, threshold) {
      ...
      // Sort by PIP descending so the strongest evidence is at the top.
      const sorted = rows.slice().sort((a, b) => b.pip - a.pip);
      const rowH = 22;
      const H = margin.top + sorted.length * rowH + margin.bottom;
      ...
      const innerH = sorted.length * rowH;
      const g = svg.append("g")
        .attr("transform", `translate(${margin.left},${margin.top})`);

      const x = d3.scaleLinear().domain([0, 1]).range([0, innerW]);
      const y = d3.scaleBand().domain(sorted.map(d => d.variable))
      ...
      sorted.forEach(d => {
        svg.append("text")
          ...
          .attr("y", margin.top + y(d.variable) + y.bandwidth() / 2 + 4)
          .attr("text-anchor", "end")
          .attr("fill", C.text)
          .attr("font-size", 11)
      ...
      sorted.forEach(d => {
        ...
        const colour = d.is_true ? C.steel : C.muted;
        const stroke = (d.pip >= threshold && !d.is_true) ? C.orange : null;
        const bar = g.append("rect")
          .attr("x", 0).attr("y", yc)
          .attr("width", x(d.pip)).attr("height", y.bandwidth())
          .attr("fill", colour)
          .style("cursor", "pointer");
        if (stroke) bar.attr("stroke", stroke).attr("stroke-width", 1.5);

        // PIP value at end of bar
        g.append("text")
          .attr("x", x(d.pip) + 4)
          .attr("y", yc + y.bandwidth() / 2 + 4)
          .attr("fill", C.text).attr("font-size", 10)
          .text(d.pip.toFixed(3));

        bar.on("mousemove", function (ev) {
          const rect = container.getBoundingClientRect();
          const truthTag = d.is_true ? "<strong style='color:#6a9bcc'>true predictor</strong>"
                                     : "<strong style='color:#8b9dc3'>noise variable</strong>";
          let extra = "";
          if (typeof d.true_coef === "number") {
            extra += `<div><span class='tooltip-key'>true coef =</span> <span class='tooltip-val'>${d.true_coef.toFixed(4)}</span></div>`;
          }
          if (typeof d.bma_mean === "number") {
            extra += `<div><span class='tooltip-key'>BMA mean =</span> <span class='tooltip-val'>${d.bma_mean.toFixed(4)}</span></div>`;
          }
          if (d.false_positive) {
            extra += `<div style='color:#d97757'><strong>false positive</strong></div>`;
          }
          tooltip.html(
            `<div>${truthTag}</div>` +
            `<div><span class='tooltip-key'>variable =</span> <span class='tooltip-val'>${d.label || d.variable}</span></div>` +
            `<div><span class='tooltip-key'>PIP =</span> <span class='tooltip-val'>${d.pip.toFixed(4)}</span></div>` +
            extra
          )
          .classed("show", true)
          .style("left", (ev.clientX - rect.left + 12) + "px")
  ...
  window.CHARTS.pip_chart = pip_chart;
})();
