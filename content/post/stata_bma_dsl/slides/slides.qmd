...
[Same data, same fixed effects — the linear term swings from −7.498 to −7.131 just by adding controls.]{.comment}
...
[Four decoys are deliberately correlated with GDP — they will look "significant" by piggybacking on GDP's real effect.]{.comment}
...
[A cubic in log GDP $G$ gives the inverted-N; $\alpha_i$ and $\delta_t$ absorb country and year heterogeneity; $\mathbf{X}^{\text{true}}$ is the 5 controls we must rediscover.]{.comment}
...
## Throw in all 12 controls and the answer barely moves — so which controls belong?
...
[Instead of betting on one horse, BMA spreads bets across the whole field.]{.comment}
...
[A variable in every high-probability model has $\text{PIP} \to 1$; one only in long-shot models stays near 0. We treat $\text{PIP} \geq 0.80$ as "robust."]{.comment}
...
[`($fe, always)` forces country and year FE into every model; `groupfv` moves the 80 country dummies as one package, not 80 independent choices.]{.comment}
...
[The second selection catches a confounder the outcome-LASSO would miss — that is the "double."]{.comment}
...
[Cluster-robust SEs, four internal LASSOs, union, then OLS — all in seconds. Wald $\chi^2 = 53.15$, $p < 0.001$.]{.comment}
