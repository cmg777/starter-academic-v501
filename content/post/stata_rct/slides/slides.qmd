...
::: {.notes}
Cash transfers are among the most common development interventions worldwide. Open with the policy stakes, then reveal the pedagogical trick: simulated data with a known true effect of 0.12 log points. The whole talk is a controlled experiment on the estimators themselves — we judge each method by how close it lands to a ground truth we planted.
:::
...
[Twelve specifications, three estimands, one truth at $0.12$ — and *every* 95% CI covers it.]{.takeaway .fragment}
...
[State the wedge early: random `treat` is exogenous; actual `D` is a choice. Most of the deck estimates the *offer* effect; Act III's coda returns to *receipt*.]{.takeaway .fragment}
...
[$p = 0.038$ flags `female`, but the SMD of $9.3\%$ is the right lens — large $n$ makes tiny gaps "significant."]{.takeaway .fragment}
...
[Overidentification test: $\chi^2(5) = 3.22$, $p = 0.667$ — no residual imbalance after weighting.]{.takeaway .fragment}
...
[Under randomization with homogeneous effects, ATE $=$ ATT. DiD will only ever give us the ATT.]{.takeaway .fragment}
...
[Randomized treatment is independent of potential outcomes, so the raw difference in means is *already* unbiased.]{.takeaway .fragment}
...
::: {.notes}
In observational studies the same controls would be doing the heavy lifting of identification. This is the single most important conceptual point for an RCT audience, and the most commonly muddled. Distinguish the randomized framing (adjustment = precision) from the observational framing (adjustment = bias removal). The estimators are identical; their *job* is not.
:::
...
[Doubly robust (AIPW / IPWRA) fits **both** — and is consistent if *either* one is correct.]{.takeaway .fragment}
...
[Belt and suspenders: if the belt fails, the suspenders hold. Only *both* wrong is fatal.]{.takeaway .fragment}
...
## RA, IPW, and DR agree because randomization made every model approximately right {.smaller}
...
[Adjusted estimates ($0.113$) sit just below the raw $0.116$ — that gap *is* the precision gain from controlling for the gender imbalance.]{.takeaway .fragment}
...
[Identification rides on **parallel trends** — plausible here because randomization equalized the groups at baseline.]{.takeaway .fragment}
...
[0.135]{.key} sits just above the cross-sectional $0.113$ — the wider SE ($0.027$ vs $0.019$) is the price of differencing.
...
[Needs only **conditional** parallel trends — covariate-specific time trends are allowed.]{.takeaway .fragment}
...
## Cross-section vs. panel: different estimands, different data, same truth inside the bars {.smaller}
...
[DiD's value is **not** a tighter SE — it is robustness to time-invariant unobservables.]{.takeaway .fragment}
...
[The doubly-robust receipt estimate ($0.117$) lands closest to the truth; both cover $0.12$.]{.takeaway .fragment}
