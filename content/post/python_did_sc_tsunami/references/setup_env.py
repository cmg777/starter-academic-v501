...
into it, registers a named Jupyter kernel (`python_did_sc_tsunami-tutorial`) that points at
...
# Pinned versions: direct imports from script.py (alphabetised), then Intel
# wheel-availability overrides (llvmlite, numba), then bootstrap (jupyter,
# ipykernel). pyfixest triggers the numba/llvmlite Intel overrides via the
# write-quarto-notebook-python catalog: numba 0.63+ and llvmlite 0.46+
# dropped macOS Intel wheels, so we pin to the last Intel-friendly pair.
PINNED: dict[str, str] = {
    "diff-diff":  "3.5.2",
    "matplotlib": "3.10.8",
    "numpy":      "2.3.5",
...
# Packages not on PyPI: installed from a pinned git commit. mlsynth provides
# the VanillaSC synthetic-control estimator used in the tutorial.
GIT_PACKAGES: dict[str, str] = {
    "mlsynth": ("mlsynth @ git+https://github.com/jgreathouse9/mlsynth.git"
                "@b7e5257a508299d9e4d10ec71ca4dc16b0096168"),
}

KERNEL_NAME = "python_did_sc_tsunami-tutorial"
KERNEL_DISPLAY = "Python Tsunami DiD+SC Tutorial"
...
def ensure_packages_in_venv() -> None:
    ...
    # git-sourced packages (not on PyPI): install if not already importable
    for dist_name, spec in GIT_PACKAGES.items():
        if installed_version(py, dist_name) is None:
...
def preflight() -> None:
    ...
    if version_unsupported:
        ...
        print(
            ...
            f"You ran setup_env.py with Python {ver} at {sys.executable}.\n\n"
            f"The pinned numpy 2.3 / pandas 3.0 / scipy 1.17 wheels require\n"
            f"Python 3.11+, and the numba/llvmlite pins cover cp311-cp313.\n"
            f"Python 3.14 has no prebuilt numba wheels yet, and Python 3.10\n"
            f"and below are beneath the minimum for these package versions.\n\n"
