...
[The estimand is the **average treatment effect (ATE)** — and we know it equals 1.0, so every method is graded against the truth.]{.takeaway .fragment}
...
[Identification (a *causal* question about the graph) stays separate from estimation (a *statistical* question about the data).]{.takeaway .fragment}
...
[The assumption is **unconfoundedness**: no unmeasured common cause of WFH and productivity. Untestable from data alone.]{.takeaway .fragment}
...
[The assumption is the **exclusion restriction**: $Z$ moves productivity *only* through WFH. True by construction here.]{.takeaway .fragment}
...
[$\beta_1$ is the causal effect — *if* the model is right and every confounder is in.]{.takeaway .fragment}
...
[IPW gives **1.0275** (SE 0.0754). It models *who gets treated*, not the outcome — so it costs a little precision.]{.takeaway .fragment}
...
[AIPW gives **1.0115** (SE 0.0623) — smallest bias of the four, precision matching regression.]{.takeaway .fragment}
...
[The instrument is strong (F = 293), yet the Wald ratio amplifies noise — SE **0.3303**, more than $5\times$ regression's.]{.takeaway .fragment}
...
[The naive CI [1.25, 1.53] is *narrow* — and excludes the truth. Precision without validity is worthless.]{.takeaway .fragment}
...
[Permute the treatment and the effect collapses to zero; add a fake confounder or drop 20% of rows and it barely moves.]{.takeaway .fragment}
