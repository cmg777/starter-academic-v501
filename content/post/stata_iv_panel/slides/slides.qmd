...
::: {.notes}
The headline puzzle. A naive OLS regression of conflict on nighttime lights returns essentially zero. That is not "no effect" — it is a contaminated reading. The whole deck is about cleaning the thermometer.
:::
...
[Region fixed effects, region-specific time trends, and year effects are absorbed via pre-detrended variables; SEs clustered on 5,689 regions.]{.comment}

::: {.notes}
A 48-... no — 5,689-region subnational panel from Hodler & Raschky (2014), taking Miguel, Satyanath & Sergenti's rainfall-IV idea below the country level. Conflict is rare: only 4.6% of region-years have any conflict death.
