...
into it, registers a named Jupyter kernel (`python_dynamic_panel-tutorial`)
...
# Substitute the dict produced in Phase 1 (probed versions + Intel overrides
# + bootstrap entries jupyter/ipykernel). Order: direct imports first
# (alphabetised), then Intel-override transitive pins (llvmlite, numba),
# then bootstrap (jupyter, ipykernel) last.
PINNED: dict[str, str] = {
    "matplotlib": "3.10.9",
    "numpy":      "2.3.5",
    "pandas":     "3.0.3",
...
KERNEL_NAME = "python_dynamic_panel-tutorial"
KERNEL_DISPLAY = "Python Dynamic Panel Tutorial"
...
def preflight() -> None:
    ...
    if version_unsupported:
        ...
        print(
            ...
            f"You ran setup_env.py with Python {ver} at {sys.executable}.\n\n"
            f"The pinned `pandas` 3.0.3 requires Python >= 3.11, the pinned\n"
            f"`pyfixest` 0.50.1 ships macOS wheels for cp311+ only, and the\n"
            f"pinned `numba`/`llvmlite` wheels cover up to cp313. Python 3.14\n"
            f"has no prebuilt numba wheels yet, and Python <= 3.10 is below\n"
            f"the minimum for these pins.\n\n"
