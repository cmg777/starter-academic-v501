...
::: {.notes}
The arc is an estimator ladder: each stage fails informatively and motivates the next. "Then we stress-test it" is the last third of the talk: the diagnostics decoder (AR(1), AR(2), Hansen, and why p ≈ 1 is a red flag), the instrument-proliferation experiment, and a digit-for-digit replication check. The meta-lesson to plant now: the winner will not be identified by any single printed statistic — only by the workflow (bracket, weak-instrument logic, proliferation experiment, replication).
:::
...
[The CI contains the whole bracket, the unit root, and explosive dynamics — all at once.]{.takeaway .fragment}
...
[0.679 sits 0.053 above the FE floor of 0.626 — within one SE of the biased bound.]{.takeaway .fragment}
...
[Even for a persistent series, last year's *change* is informative about this year's *level* — at the price of one new assumption, **mean stationarity**.]{.takeaway .fragment}

::: {.notes}
Mean stationarity, stated plainly: firms' initial deviations from their steady states must be unrelated to α_i. System GMM stacks the differenced equation (keeping its Arellano-Bond instruments) with the original levels equation, instrumented by lagged differences. The lake analogy: learning a lake's depth from ripples alone (differences) vs also using waterline marks on the shore (levels) — when the lake is calm, the ripples carry almost no information. Mean stationarity is untestable directly, but the Hansen test gets indirect bite because the levels moments are overidentifying. We also collapse instruments to 32 — safely below Roodman's rule that instruments should not outnumber the 140 firms.
:::
...
[Every box is ticked — which is exactly why two of these three tests deserve a second look.]{.takeaway .fragment}

::: {.notes}
Wage and capital elasticities also sharpen: short-run wage elasticity −0.816 (SE 0.276), capital 0.589 (SE 0.172). Resist reporting the implied long-run wage elasticity (β1+β2)/(1−ρ) ≈ −2.5 without a warning — its denominator 1−ρ ≈ 0.073 makes it explosively fragile. But why is AR(1) rejecting "as required"? And is a bigger Hansen p always better? That is the next slide — two of these three tests are routinely read backwards.
:::

## The diagnostics decoder: two of the three tests are read backwards {.smaller}
...
[AR(1) rejecting is the model working, not failing; a Hansen p near 1 is the test overwhelmed, not the instruments vindicated.]{.takeaway .fragment}
...
[Same model, six ways — five shown: uncollapsed 2:3 is *rejected* (p = 0.035) while its collapsed twin *passes*, driven purely by instrument count.]{.takeaway .fragment}
...
[Exact match under a hard assertion — the NumPy-2 compatibility shim perturbs nothing.]{.takeaway .fragment}
