...
[A *lagged dependent variable* on the right-hand side while the firm effect $\alpha_i$ sits in the error: $n_{i,t-1}$ depends on $\alpha_i$ by construction, so the regressor is correlated with the error no matter how many controls we add.]{.comment}
