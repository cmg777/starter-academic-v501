...
(function () {
  ...
  function selection_bars(container) {
    ...
    function update(data, activeOutcomes) {
      ...
      // Discover the set of methods present in the data (allows reuse for non-DL apps).
      ...
      outcomes.forEach((outcome, oi) => {
        ...
        const x = d3.scaleBand().domain(methodDomain).range([0, facetW]).padding(0.45);
        ...
        sub.forEach(d => {
          const xc = x(d.method);
          const w  = x.bandwidth();
          const fill = orangeMethods.has(d.method) ? C.orange
                     : tealMethods.has(d.method)   ? C.teal
                     : C.steel;
          facet.append("rect")
