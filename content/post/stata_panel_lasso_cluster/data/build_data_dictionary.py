...
def load_frame(fname, ext):
    ...
    if ext == ".csv":
        ...
        try:
            ...
            # Some legacy .dta files store labels in a non-UTF-8 codepage; fall
            # back to latin1 (covers windows-1252/iso-8859-1) so reading succeeds.
            df, m = pyreadstat.read_dta(path, encoding="latin1")
        labels = dict(m.column_names_to_labels or {})
    else:
        raise ValueError(ext)
    for c in df.columns:
        if not pd.api.types.is_numeric_dtype(df[c]):
            # Coerce non-numeric columns to clean strings (preserving NaN) so a
            # mixed int/str object column never breaks pyreadstat.write_dta.
