...
The resource curse hypothesis holds that natural-resource abundance can depress development, with institutional quality determining whether resource wealth becomes a blessing or a curse — a debate that causal machine learning has recently sharpened. This tutorial aims to replicate the three core findings of Hodler, Lechner and Raschky (2023) — that mining raises development and conflict, that mineral-price effects are non-linear, and that institutions moderate mining but not prices — using Stata 19's `cate` command on data with known ground-truth effects. It uses a simulated balanced panel of 3,000 district-year observations (300 districts over 10 years, 2003–2012, across 8 fictional countries), with log nighttime lights and a conflict indicator as outcomes and executive constraints and quality of government as institutional moderators. The analysis estimates average, group, and individualized treatment effects for six pairwise binary treatment contrasts via generalized random forests, comparing the Partialing-Out (PO) and doubly robust Augmented IPW (AIPW) estimators with 5-fold cross-fitting, supported by formal heterogeneity tests. Mining significantly raises nighttime lights (AIPW ATE = 0.149 for the 1-0 contrast, PO = 0.194) and conflict (AIPW = 0.066, about 6.5 percentage points above a 10.7% baseline), price effects are non-linear (medium-vs-low = -0.011, p = 0.90; high-vs-low = 0.405, p < 0.001), and institutions systematically moderate the mining effect (GATE falls from 0.275 at the weakest executive constraints to 0.051 at the strongest; chi2(5) = 96.90, p < 0.001) but not the price premium (chi2(3) = 5.81, p = 0.121). These results show that Stata 19's `cate` recovers heterogeneous causal structure natively, letting researchers identify which subgroups treatment helps most without external packages.
