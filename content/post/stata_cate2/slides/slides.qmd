...
[A single ATE is just $E\{\tau(\mathbf{X})\}$ — this function averaged over everyone, discarding exactly the heterogeneity we want.]{.takeaway .fragment}
...
[Because the data-generating process is known, every estimate can be scored against its true value — a luxury real data never gives.]{.takeaway .fragment}
...
[Stata's `cate` needs a *binary* treatment, so we subset to two groups at a time and run a generalized random forest on each contrast.]{.takeaway .fragment}
...
[Some confounders push the raw gap above the truth, some below — geography, institutions, and wealth all differ across mining status.]{.takeaway .fragment}
...
[The mining effect is **more than three times larger** in weakly-governed districts — the same slope, now as a policy-ready contrast.]{.takeaway .fragment}
