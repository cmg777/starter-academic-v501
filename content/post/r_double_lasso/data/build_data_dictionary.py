...
def build_do(order, frames, cms_all, fms):
    ...
    for b in order:
        ...
        name_map = stata_name_map(frames[b].columns)
        for c in frames[b].columns:
            m = cms_all[b][c]
            bits = [m["defn"]]
            if m["constr"]:
            ...
            lines.append(f'note {name_map[c]}: {ascii_safe(txt)}')
...
def stata_safe_name(name, used):
    """Sanitize a column name to a valid, unique Stata variable name (<=32 chars,
    letters/digits/underscore, starting with a letter/underscore). Non-syntactic
    characters from the Matlab control names (e.g. '^', '*') become underscores.
    Only the .dta variable names are affected; the source CSVs and the HTML/stats
    ...
    while cand.lower() in used:
        suffix = "_" + str(i)
        cand = s[:32 - len(suffix)] + suffix
        i += 1
    used.add(cand.lower())
    return cand


def stata_name_map(columns):
    """Build an ordered original-name -> Stata-safe-name map for one dataset."""
    used = set()
    return {c: stata_safe_name(c, used) for c in columns}


def write_dta(base, df, cms):
    name_map = stata_name_map(df.columns)
    labels = [cms[c]["label"] for c in df.columns]
    vvl = {}
    for c in df.columns:
        safe = name_map[c]
        if cms[c]["value_labels"]:
            vvl[safe] = {int(k): v for k, v in cms[c]["value_labels"].items()}
        elif cms[c]["kind"] == "dummy":
            vvl[safe] = YESNO
    df_out = df.rename(columns=name_map) if any(k != v for k, v in name_map.items()) else df
    pyreadstat.write_dta(df_out, os.path.join(BASE, base + ".dta"), file_label=fms_global[base]["label"],
