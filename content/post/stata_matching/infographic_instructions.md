...
The top-center panel (row 1, column 2) is titled "THE CONFOUNDING PROBLEM" in steel blue chalk small-caps with a "2" circle in warm orange. Two chalk-drawn profile cards sit side by side in the center of the panel. The left card is labeled "SMOKERS (n=864)" in chalk white small-caps and lists four short chalk-white bullet points beneath the header: "Younger", "Less educated", "Late prenatal care", "Less often married". The right card is labeled "NON-SMOKERS (n=3,778)" in chalk white small-caps and lists the mirror bullets: "Older", "More educated", "Early prenatal care", "More often married". Between the two cards sits a large chalk "≠" symbol in warm orange (#e8956a), making the visual claim that these two groups are not comparable on observable traits. The phrase "Naive gap = −275 g (biased)" appears below the cards in large warm orange chalk numerals with a chalk underline. In chalk white, the body text reads: "Smokers and non-smokers differ on more than smoking — these differences leak into the raw gap." and "Adjusting for these covariates is what every method below buys us." A chalk arrow connects to Panel 3 with the phrase "How do we fix it?" in small muted gray (#b0a89a) chalk along the arrow.
...
Panel 2 (top-center) "THE CONFOUNDING PROBLEM": two side-by-side chalk profile cards labeled "SMOKERS (n=864)" and "NON-SMOKERS (n=3,778)" each with 4 covariate bullets (younger/older, less/more educated, late/early prenatal care, less/more often married); large warm orange "≠" symbol between them; callout "Naive gap = −275 g (biased)".
...
- **Body sentences**:
  - "Smokers and non-smokers differ on more than smoking — these differences leak into the raw gap."
  - "Adjusting for these covariates is what every method below buys us."
- **Icon**: two chalk-drawn profile cards (smokers vs. non-smokers) flanking a large warm orange ≠ symbol
- **Mini-viz**: two profile cards listing four covariates each in mirrored bullet form (younger/older, less/more educated, late/early prenatal care, less/more often married); chalk ≠ symbol between them in warm orange (#e8956a)
