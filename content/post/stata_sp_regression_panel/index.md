...
For further study, consider applying these methods to other spatial datasets or exploring alternative spatial specifications. The companion tutorial on [cross-sectional spatial regression]({{< ref "/post/stata_sp_regression_cross_section" >}}) covers the spatial models available for single-period data, including the full taxonomy of SAR, SEM, SLX, SDM, SDEM, and SAC models. For Python implementations of spatial econometrics, see the PySAL ecosystem and the `spreg` package.
