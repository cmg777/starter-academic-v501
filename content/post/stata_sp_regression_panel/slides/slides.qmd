...
[Strongly balanced Baltagi panel, 1963–1992. Two states are neighbors if they share a border; the diagonal of $W$ is zero (no self-loop).]{.takeaway .fragment}
...
[`norm(row)` makes each row of $W$ sum to 1, so $Wy$ for California averages Nevada, Oregon, and Arizona's consumption.]{.takeaway .fragment}
...
[$R^2 = 0.224$. This naive benchmark ignores the panel structure entirely — every state treated as an independent draw.]{.takeaway .fragment}
...
[Within $R^2 = 0.789$ — state *and* year effects absorbed. But every state still depends only on its **own** price and income.]{.takeaway .fragment}
...
[$\rho W y$ is the neighbor-consumption feedback; $W X \theta$ is the spillover of neighbors' prices and incomes; $\mu_i,\lambda_t$ are state and year effects.]{.takeaway .fragment}
...
[`type(both)` = two-way FE; `mod(sdm)` = Spatial Durbin; `effects` decomposes the impact into direct + indirect via 999 Monte Carlo draws.]{.takeaway .fragment}
...
[Higher neighbor prices also cut local consumption — exactly the cross-border-shopping signature.]{.takeaway .fragment}
...
[The spillover is **as large as** the own-state effect — when neighbors raise prices 1%, local consumption falls about as much as if the state had raised its own price.]{.takeaway .fragment}
...
[With $T = 30$, the incidental-parameters bias of order $1/T$ is small; the near-identical estimates confirm the ML results are reliable.]{.takeaway .fragment}
...
[Spatial dependence in cigarette demand is *substantive*, not a nuisance — neighbors' consumption, prices, and incomes all matter.]{.takeaway .fragment}
...
[The static SDM conflates short- and long-run responses into one coefficient; the dynamic model separates how *fast* a tax bites from how *hard* it eventually bites.]{.takeaway .fragment}
