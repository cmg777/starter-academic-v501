...
::: {.notes}
The hook. Kuznets is one of the most-tested hypotheses in development economics. The whole deck stress-tests his shape against modern panel data. Plant the question; we resolve it in Act III.
:::
...
[Periods are 5-year averages from 1990–1994 through 2010–2013; mean Gini $= 0.064$, SD $= 0.033$.]{.comment}
...
[$\beta_2$ lets the curve bend once (an inverted-U if negative); $\beta_3$ lets it bend a *second* time (an N if positive). A linear term alone forces monotonicity.]{.comment}
...
[Cubic terms are only marginally significant ($p \approx 0.07$–$0.09$), and $R^2$ barely moves across the ladder.]{.comment}
...
[What's left is the *within* estimator: how each country's inequality moves as it develops, net of fixed traits and global shocks.]{.comment}
...
## The honest fit is the within-R², and it rises 16× from linear to cubic
...
[A researcher who fit only the linear FE model would conclude development has *no* effect — a false negative born of forcing a straight line through an N.]{.comment}
...
[Fixed effects strip time-invariant confounders, but the relationship is descriptive: read it as conditional association, not a policy lever.]{.comment}
