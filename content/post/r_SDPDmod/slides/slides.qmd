...
[Log-log: every coefficient is an elasticity — % change in consumption per % change in price.]{.takeaway .fragment}
...
[`blmpSDPD()` scores every model at once with a log-marginal posterior — no ad-hoc sequence of Wald tests.]{.takeaway .fragment}
...
[Once the lagged outcome enters, the spatial signal weakens — the data can no longer tell the spatial models apart.]{.takeaway .fragment}
...
[For short panels ($T < 10$) the correction would matter far more; here it mainly rescues $\rho$.]{.takeaway .fragment}
...
[The income spillover is *negative* (−0.20): richer neighbours pull your consumption *down* — a channel the SAR cannot see.]{.takeaway .fragment}
...
[A static $\rho$ must absorb *both* spatial spillover and serial correlation — so it is biased upward.]{.takeaway .fragment}
...
[Habit persistence is the dominant force; the static SDM was overstating contemporaneous spatial dependence.]{.takeaway .fragment}
...
[The spatial lag of consumption hid the effect; only proper dynamics reveal it.]{.takeaway .fragment}
