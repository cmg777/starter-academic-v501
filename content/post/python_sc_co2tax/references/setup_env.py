...
into it, registers a named Jupyter kernel (`python_sc_co2tax-tutorial`) that
...
# Direct imports from script.py (alphabetised), then Intel-override transitive
# pins (llvmlite, numba) — last releases with macOS Intel wheels; pulled in
# transitively via pyfixest / statsmodels so we avoid a slow source build on
# Intel macOS — then bootstrap entries (jupyter, ipykernel).
PINNED: dict[str, str] = {
    "matplotlib":  "3.8.3",
    "numpy":       "1.26.4",
    "pandas":      "2.2.1",
...
KERNEL_NAME = "python_sc_co2tax-tutorial"
KERNEL_DISPLAY = "Python SC CO2 Tax Tutorial"
