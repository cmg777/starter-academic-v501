...
[The long pre-period is a structural advantage. It lets us check the counterfactual *before* the policy — where the gap should be zero.]{.takeaway .fragment}
...
[$Y^{\text{obs}}$ is what happened; $\hat Y^{N}$ is the counterfactual; the gap $\tau_t$ is the treatment effect on the treated.]{.takeaway .fragment}
...
[Differencing against a control flips +0.55 to negative — but one control is underpowered, and the pre-trends don't line up.]{.takeaway .fragment}
...
[Non-negative weights that sum to one $\Rightarrow$ the synthetic is a convex blend — no extrapolation, no parallel-trends assumption.]{.takeaway .fragment}
...
[If oil firms had absorbed the tax, the price signal never reaches drivers. They didn't — so the behavioural channel is real.]{.takeaway .fragment}
...
[Salience + permanence: a tax is announced, debated, and persistent; a price drift is just a number on a billboard.]{.takeaway .fragment}
