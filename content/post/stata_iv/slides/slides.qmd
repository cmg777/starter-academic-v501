...
[$Y$ is log GDP, $X$ is institutional quality, $U$ collects every unobserved driver of income. The non-zero covariance is exactly what biases OLS.]{.takeaway .fragment}
...
[Settler mortality must shift institutions (we can check) and reach 1995 GDP only through them (we must assume).]{.takeaway .fragment}
...
[Stage 1 regresses $X$ on $Z$; stage 2 regresses $Y$ on the *predicted* $\hat X$. With one instrument, the IV estimate is just reduced form $\div$ first stage.]{.takeaway .fragment}
...
[This is the number the IV will stress-test — not one it generates.]{.takeaway .fragment}
...
[Borderline on the conventional threshold; the weak-IV-robust AR test reassures.]{.takeaway .fragment}
...
[`(avexpr = logem4)` declares the endogenous regressor and its instrument; `endog()` runs the Durbin-Wu-Hausman test.]{.takeaway .fragment}
...
[Durbin-Wu-Hausman $\chi^2 = 9.09$ ($p = 0.003$) rejects OLS consistency.]{.takeaway .fragment}
...
[No control set drives the coefficient to zero or flips its sign.]{.takeaway .fragment}
