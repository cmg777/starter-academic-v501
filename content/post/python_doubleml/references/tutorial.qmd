...
subtitle: "Estimating the causal effect of a cash bonus on unemployment duration using DML with the Pennsylvania Bonus Experiment"
author: "Carlos Mendez"
date: "2026-03-10"
jupyter: python_doubleml-tutorial
format:
  html:
    theme: cosmo
...
In this post a Random Forest with 500 trees and max_depth = 5 fits both $g$ and $m$. The RCT means $m(X)$ is roughly constant (the assignment probability), but $g(X)$ still absorbs predictive variation in `inuidur1`.
...
**5. Neyman orthogonality** $E[\partial_\eta \psi] = 0$. The score function $\psi$ has zero expected gradient with respect to the nuisance parameters $\eta$ at the truth. Means small ML errors in $\hat\eta$ do not bias $\hat\theta$.
...
#| fig-cap: "Distribution of log unemployment duration by treatment group."
...
#| fig-cap: "Covariate balance between control and bonus groups."
...
#| fig-cap: "Coefficient comparison across all estimation methods."
...
```{python}
#| label: fig-confint
#| fig-cap: "95% confidence intervals for DoubleML estimates."
...
- Companion script: [`script.py`](https://raw.githubusercontent.com/cmg777/starter-academic-v501/master/content/post/python_doubleml/script.py)
- Published post: <https://carlos-mendez.org/post/python_doubleml/>
