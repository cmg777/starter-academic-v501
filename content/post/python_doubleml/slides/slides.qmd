...
[Treatment was *assigned at random*, so $D$ is independent of $X$ by design — the estimand is a clean **Average Treatment Effect (ATE)**.]{.takeaway .fragment}
...
[A linear $X_i'\gamma$ can miss nonlinear structure in $X \to Y$. The leftover variation widens the standard errors.]{.takeaway .fragment}
...
[Naive OLS: **−0.0855**.  With covariates: **−0.0717**. The shift is precision, not bias.]{.takeaway .fragment}
...
[$\theta_0$ is the causal **ATE**. $g_0(X)$ and $m_0(X)$ are *nuisance* functions — scaffolding we estimate but don't report.]{.takeaway .fragment}
...
[Like noise-canceling headphones: ML learns how $X$ drives $Y$ and $D$, we subtract it, and only the $D \to Y$ signal is left.]{.takeaway .fragment}
...
[Split into $K=5$ folds; each fold's residuals come from models trained on the *other four*, then average.]{.takeaway .fragment}
...
[Two utterly different learners, a 0.0024 gap — under 7% of one standard error.]{.takeaway .fragment}
