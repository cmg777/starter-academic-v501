...
(function () {
  ...
  function forest_plot(container) {
    ...
    const colorMap = {
      ...
      "DL (rigorous)":      C.teal,
      "DL (CV)":            C.orange,
      // DoubleML post extensions
      "Naive OLS":          C.muted,
      "OLS + Covariates":   C.steel,
      "DoubleML (RF)":      C.teal,
      "DoubleML (Lasso)":   C.orange,
    };
    ...
    function update(data, activeMethods, activeOutcomes) {
      ...
      outcomes.forEach((outcome, oi) => {
        ...
        const ext = d3.extent(subset.flatMap(d => [
          d.ci_lo == null ? d.estimate : d.ci_lo,
          d.ci_hi == null ? d.estimate : d.ci_hi
        ]));
        ...
        subset.forEach(d => {
          ...
          const hasCI = d.ci_lo != null && d.ci_hi != null;
          ...
          g.on("mousemove", function (ev) {
            ...
            const seTxt = (d.se === null || d.se === undefined) ? "—" : (+d.se).toFixed(4);
            const ciTxt = (d.ci_lo === null || d.ci_lo === undefined || d.ci_hi === null || d.ci_hi === undefined)
              ? "—"
              : `[${(+d.ci_lo).toFixed(3)}, ${(+d.ci_hi).toFixed(3)}]`;
            const nselTxt = (d.n_selected === null || d.n_selected === undefined)
              ? "0 (no controls)"
              : d.n_selected;
            tooltip.html(
              ...
              `<div><span class='tooltip-key'>SE =</span> <span class='tooltip-val'>${seTxt}</span></div>` +
              `<div><span class='tooltip-key'>95% CI =</span> <span class='tooltip-val'>${ciTxt}</span></div>` +
              `<div><span class='tooltip-key'>controls used =</span> <span class='tooltip-val'>${nselTxt}</span></div>`
            )
            .classed("show", true)
