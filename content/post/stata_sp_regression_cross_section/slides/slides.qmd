...
[On the OLS residuals, $I = 0.222$ ($z = 2.84$, $p = 0.005$): high-crime tracts neighbour high-crime tracts. The i.i.d.-error assumption fails.]{.takeaway .fragment}
...
[Row-standardized means each row of $W$ sums to 1, so the spatial lag $W y$ is the *weighted average* among a tract's neighbours — about 4.8 of them.]{.takeaway .fragment}
...
[$W\cdot INC$ and $W\cdot HOVAL$ are pre-computed in Mata, then entered as ordinary regressors in SLX/SDM/SDEM/GNS.]{.takeaway .fragment}
...
[Anselin's rule favours **SEM** here — but the full taxonomy will tell a subtler story.]{.takeaway .fragment}
...
[$\rho = 0.428$ ($z = 3.49$, $p < 0.001$): a contagion channel. Because $W y$ is endogenous, we fit by maximum likelihood, not OLS.]{.takeaway .fragment}
...
[$\lambda = 0.562$ ($z = 4.23$, $p < 0.001$). Spatial dependence is a *nuisance* in the error — so the SEM yields **zero** spillover effects by construction.]{.takeaway .fragment}
...
[$W\cdot INC = -1.40$ ($p = 0.016$): richer neighbours mean less own crime. No spatial multiplier, so $\theta$ *is* the indirect effect.]{.takeaway .fragment}
...
[The SDM nests SAR ($\theta = 0$), SLX ($\rho = 0$), and SEM (common-factor) — so we can *test down* from it.]{.takeaway .fragment}
...
[Direct effects stay near the other models; the indirect income effect is **larger** than in SAR ($-0.76$) because SDM counts *both* channels.]{.takeaway .fragment}
...
[Only $\rho$ is indispensable; $\theta$ and $\lambda$ survive on economics, not on a Wald test.]{.takeaway .fragment}
...
[Direct effects are stable everywhere; the *spillover* is where the models disagree — and SLX/SDM/SDEM/GNS all say it is large and negative.]{.takeaway .fragment}
...
[Seven spatial parameters chasing 49 observations: $\rho$, $\lambda$, and the $\theta$'s are only *weakly identified* together, so every one goes insignificant.]{.takeaway .fragment}
