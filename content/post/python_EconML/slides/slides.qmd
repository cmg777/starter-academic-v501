...
[Mining districts differ systematically — worse geography, weaker institutions — so a raw difference-in-means confounds selection with effect.]{.takeaway .fragment}
...
[Where $\tau(\cdot)$ bends with $\mathbf{x}$, mining helps some districts more than others — that bend is the whole point.]{.takeaway .fragment}
...
[Structure mirrors Hodler, Lechner & Raschky (2023); because we built the data, the identifying assumption holds by construction.]{.takeaway .fragment}
...
[Subtract the predictable parts: $\tilde Y_i = \tau(\mathbf{X}_i)\,\tilde T_i + \varepsilon_i$. The nuisance functions $g_0, m_0$ exist only to be subtracted out.]{.takeaway .fragment}
...
[At the truth, the estimating equation is *flat* in the nuisances. A 10% error in $\hat g_0$ enters $\hat\tau$ at order $(0.10)^2 \approx 0.01$ — second order.]{.takeaway .fragment}
...
[Conditional Independence: once we know a district's geography, institutions, country, and year, mining status is as good as random. *We built the data, so it holds here — in real data it is untestable.*]{.takeaway .fragment}
...
[Flat from low to medium, then a jump at high prices — shape discovery with no functional form pre-specified.]{.takeaway .fragment}
...
[Quality of government tells the same story as executive constraints — robust to the institutional measure.]{.takeaway .fragment}
...
[Response.]{.rebuttal} Exactly right. $\tau(\mathbf{x})$ is identified *only* under the Conditional Independence Assumption — the forest earns honest intervals, but it cannot rule out an unobserved confounder.
