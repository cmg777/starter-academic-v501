...
[Source: Global Data Lab (Smits & Permanyer, 2019). Mean SHDI rose only $+0.0053$ overall — yet income *fell* in 71 of 153 regions (46.4%). The aggregate calm hides spatial turbulence.]{.comment}
...
[We use Queen — appropriate for irregular administrative borders. Then row-standardize ($w_{ij}\to w_{ij}/\sum_j w_{ij}$) so each row sums to 1.]{.comment}
...
[For each pair of neighbours it multiplies their deviations from the mean. High-next-to-high (and low-next-to-low) makes the products positive — so $I>0$ means clustering.]{.comment}
...
[A permutation test reshuffles all SHDI values across regions 999 times — like dealing cards to random seats — and asks how often a random map beats the real one.]{.comment}
...
[Expected $I$ under randomness $= -0.0066$. The observed values are an order of magnitude away — and 2019 is *higher* than 2013.]{.comment}
...
[Each region's own standardized value $z_i$ times the average of its neighbours' — large and positive in a cluster's core, negative for a spatial outlier.]{.comment}
...
[Significance is permutation-based: only regions with $p<0.10$ are coloured; the rest stay grey.]{.comment}
...
::: {.notes}
At p < 0.10: 30 HH, 37 LL, 5 HL, 1 LH, 80 not significant. The HH core is anchored by R. Metropolitana (CHL), C. Buenos Aires (ARG), Antofagasta (CHL); the LL core by three Guyanese regions. San Andrés (COL) is the lone HL — a high-development island among lower mainland neighbours. The clusters cover roughly half the map.
:::
...
[Prosperity clusters stable; deprivation cluster expanding by 8 regions. Asymmetric evolution — the signature of a localized crisis.]{.comment}
...
[Bolivia's arrows point right — own development improved — but its neighbours stayed poor, so it remained inside the LL cold spot.]{.comment}
