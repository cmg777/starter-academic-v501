...
::: {.callout-warning}
## First-time setup for Positron (read this before clicking **Render**)

The package-installation chunk further down (`setup-packages`) runs *inside* the Jupyter kernel, so Jupyter must already exist in your Python before Quarto can start the render. Hitting **Render** on a fresh machine without doing the steps below will fail with `ModuleNotFoundError: No module named 'yaml'` or `Jupyter is not available in this Python installation`.

You only need to do this once.

**1. Install [Miniconda](https://docs.conda.io/projects/miniconda/) or Anaconda** if you do not already have a `conda` Python on your machine.

**2. Install Jupyter into your conda environment.** Open a terminal:

- **macOS:** in Positron, `Terminal → New Terminal`, or use the system Terminal app.
- **Windows:** open `Anaconda Prompt` from the Start menu (not regular `cmd`).
...
**3. Point Positron at the right Python.** In Positron, open the command palette (`Cmd+Shift+P` on macOS, `Ctrl+Shift+P` on Windows) → `Python: Select Interpreter` → choose the conda Python from step 1 (its path contains `miniconda3` or `anaconda3`). The status bar at the bottom of Positron should then display that interpreter.

**4. Click Render.** The `setup-packages` chunk will install `numpy`, `pandas`, `matplotlib`, and `pyfixest` at the exact versions used to build this tutorial. First render takes ~2 minutes; subsequent renders are nearly instant.

**Still stuck?** Run `import sys; print(sys.executable)` in any Python chunk. The path should contain `miniconda3` or `anaconda3`. If it instead says `/Library/Developer/CommandLineTools/usr/bin/python3` (macOS) or `C:\Python3X\python.exe` (Windows), step 3 was not applied — re-run *Python: Select Interpreter* and restart Positron if necessary.
:::
