...
::: {.callout-tip}
## First render: ~2 minutes (one-time setup)

The first time you click **Render**, Quarto runs `setup_env.py` (see `_quarto.yml`) which creates a hermetic `.venv/` next to this notebook and pip-installs pinned packages (`numpy`, `pandas`, `matplotlib`, `pyfixest`, `jupyter`, `ipykernel`). It then registers a Jupyter kernel named `pyfixest-tutorial` pointing at that `.venv/`. Subsequent renders skip the install and run in seconds.

**Prerequisites:** Python 3.10+ and a working `quarto` install. That's it. No manual `conda install`, no `Python: Select Interpreter` step.

**Troubleshooting.** If the first render aborts with `python3: command not found` (some Windows installs lack the `python3` shim), open `_quarto.yml` and replace `python3` with `python`, then click Render again.
:::
...
The first chunk verifies that the kernel is running inside the `.venv/` that `setup_env.py` created and that pinned package versions match — the actual install happens in the pre-render step (see `_quarto.yml`), so this chunk is fast on every render.
...
# Pinned versions used to produce the published numbers. The .venv/ next
# to this notebook was created by setup_env.py via Quarto's pre-render
# hook. Here we confirm the kernel is the venv kernel and versions match.
PINNED = {
    "numpy":      "1.26.4",
    "pandas":     "2.2.2",
    "matplotlib": "3.9.2",
...
assert ".venv" in py.parts, (
    ...
    "Re-render so the pre-render hook activates the venv kernel."
)
print(f"Python: {py}")
