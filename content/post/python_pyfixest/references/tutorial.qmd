...
**Before clicking Render**, open a terminal *in this folder* and run:

```bash
python3 setup_env.py
```

This creates a hermetic `.venv/` next to this notebook, pip-installs pinned packages (`numpy`, `pandas`, `matplotlib`, `pyfixest`, `numba`, `llvmlite`, `jupyter`, `ipykernel`), and registers a Jupyter kernel named `pyfixest-tutorial`. First run takes ~2 minutes; the script is idempotent — re-running it later is a fast no-op.

After it finishes, open this folder in Positron and click **Render**. Subsequent renders are instant.

**Prerequisites:** Python 3.10+ and a working `quarto` install.
