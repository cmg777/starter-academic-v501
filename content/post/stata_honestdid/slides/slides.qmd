...
[Observational, not randomized: states *chose* to expand, so parallel trends is a genuine concern — exactly the worry `honestdid` quantifies.]{.takeaway .fragment}
...
[Control rose 6.46 pp; treated rose 12.64 pp; the difference is the policy effect.]{.takeaway .fragment}
...
[$t = 7.24$, $p < 0.001$, 95% CI $[4.45,\ 7.91]$ pp · clustered on 38 states.]{.takeaway .fragment}
...
[We never observe the true $\delta_s$ — the package uses the estimated pre-period coefficients and their uncertainty to build valid CIs.]{.takeaway .fragment}
...
[Needs $\ge 2$ pre-periods (three points define one acceleration) — unavailable in the 2x2, unlocked by the full panel.]{.takeaway .fragment}
...
[breakdown value under $\Delta^{RM}$ · the post-violation must be 1.5–2x the worst pre-deviation to overturn the result]{.bignum-label}
...
[Callaway-Sant'Anna ATT agrees with TWFE here because we held a single 2014 cohort — a clean robustness check.]{.takeaway .fragment}
