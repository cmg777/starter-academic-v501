...
subtitle: "Building a comparable Human Development Index across two time periods using pooled PCA on real sub-national data for 153 South American regions"
author: "Carlos Mendez"
date: "2026-03-21"
jupyter: python_pca2-tutorial
format:
  html:
    theme: cosmo
...
In the [Introduction to PCA Analysis for Building Development Indicators](https://carlos-mendez.org/post/python_pca/), we built a Health Index from two indicators using a six-step pipeline. That tutorial's Discussion section raised a critical warning:
...
**3. Variance explained** $\lambda_k / \sum \lambda$.
...
**6. PC1 score (composite index)** $s_i = X_i v_1$.
...
The pooled pipeline extends the [six-step pipeline from the previous tutorial](https://carlos-mendez.org/post/python_pca/#2-the-pca-pipeline) by adding a stacking step at the beginning and replacing per-period parameters with pooled parameters at the standardization, covariance, and normalization steps.
...
The analysis uses the same libraries as the [previous tutorial](https://carlos-mendez.org/post/python_pca/): NumPy for linear algebra, pandas for data management, matplotlib for visualization, and scikit-learn for verification. It also adds [SciPy](https://scipy.org/) for Spearman correlation, [GeoPandas](https://geopandas.org/) + [mapclassify](https://pysal.org/mapclassify/) + [contextily](https://contextily.readthedocs.io/) for spatial maps, and [PySAL's inequality](https://pysal.org/inequality/) module for the Gini index.
...
#| fig-cap: "Pooled correlation heatmap of the three HDI sub-indices."
...
The correlations are moderate to strong but far from the near-perfect values we saw in the [previous tutorial's](https://carlos-mendez.org/post/python_pca/) simulated data ($r > 0.93$). Education and Income show the strongest correlation (0.68), followed by Health and Income (0.63), with Education and Health the weakest (0.44). These lower correlations mean PCA will capture less variance in PC1 --- the three indicators carry more independent information than in the simulated case, reflecting the genuine complexity of human development. The weak Education-Health link (0.44) suggests that a region can have high literacy but mediocre life expectancy (or vice versa) --- education and health are partly independent dimensions of development.

```{python}
#| label: fig-period-shift-scatter
#| fig-cap: "Scatter plot of Education vs Income colored by period, showing education rising but income declining."
...
```{python}
#| label: fig-perperiod-weights
#| fig-cap: "Grouped bar chart showing per-period eigenvector weights shifting between 2013 and 2019."
...
```{python}
#| label: fig-perperiod-rank-shift
#| fig-cap: "Slopegraph showing 10 regions with the largest rank shifts under per-period PCA."
...
The off-diagonals range from 0.44 (Education-Health) to 0.68 (Education-Income). These are substantially lower than the 0.93--0.95 values in the [simulated data from the previous tutorial](https://carlos-mendez.org/post/python_pca/#8-step-3-the-covariance-matrix----mapping-the-overlap), reflecting the genuine complexity of human development. Education and Health are only moderately correlated because they measure different dimensions --- a region can have high literacy but mediocre life expectancy (or vice versa). This means PC1 will capture less total variance, and the eigenvector weights will be more unequal.
...
```{python}
#| label: fig-pooled-hdi-bars
#| fig-cap: "Paired horizontal bar chart showing top and bottom 15 regions with 2013 and 2019 HDI."

# Build a wide-format table for the paired bar chart
df_pooled_p1 = df[df["period"] == "Y2013"].copy()
df_pooled_p2 = df[df["period"] == "Y2019"].copy()
df_wide = df_pooled_p1[["region", "country", "region_country", "hdi"]].rename(
    columns={"hdi": "hdi_p1"}
).merge(
...
labels_bars = list(show_df["region_country"])
...
```{python}
#| label: fig-pooled-vs-perperiod-change
#| fig-cap: "Scatter plot of pooled vs per-period HDI change with 45-degree agreement line."
...
```{python}
#| label: fig-rank-comparison-bump
#| fig-cap: "Bump chart comparing improvement rankings under pooled vs per-period PCA."
...
```{python}
#| label: fig-validation-vs-shdi
#| fig-cap: "Side-by-side scatter plots comparing pooled PCA and per-period PCA against the official SHDI."
...
```{python}
#| label: fig-validation-changes
#| fig-cap: "Side-by-side scatter plots comparing pooled and per-period PCA HDI changes against official SHDI changes."
...
The pooled PCA pipeline with scikit-learn is nearly identical to the [single-period pipeline from the previous tutorial](https://carlos-mendez.org/post/python_pca/#12-replicating-the-analysis-with-scikit-learn). The key insight is that sklearn's `fit_transform` on the stacked data IS pooled PCA --- no special panel-data library is needed.
...
#| fig-cap: "Side-by-side choropleth maps of pooled PCA HDI for 2013 and 2019 with fixed Fisher-Jenks breaks."
...
```{python}
#| label: fig-gini-dynamics
#| fig-cap: "Grouped bar chart showing Gini index for each indicator in 2013 and 2019."
...
```{python}
#| label: fig-gini-weighted-comparison
#| fig-cap: "Side-by-side comparison of unweighted and population-weighted Gini indices."
...
1. [Mendez, C. (2026). Introduction to PCA Analysis for Building Development Indicators.](https://carlos-mendez.org/post/python_pca/)
...
- Companion script: [`script.py`](https://raw.githubusercontent.com/cmg777/starter-academic-v501/master/content/post/python_pca2/script.py)
- Published post: <https://carlos-mendez.org/post/python_pca2/>
