...
[With $T = 2$, every worker contributes exactly two rows — the cleanest setting to see that first-differences and the within estimator are the *same thing*.]{.takeaway .fragment}
...
[Hausman and the Mundlak term are the formal tests for choosing between the two camps.]{.takeaway .fragment}
...
[Highly significant ($t \approx 3.25$) — and almost certainly biased if ability selects out of union jobs.]{.takeaway .fragment}
...
[FDFE $= 0.2113$ (SE 0.079); the SE is $3.4\times$ larger than POLS — the switcher-only signature.]{.takeaway .fragment}
...
[Within transformation, first-differences, and dummy-variable FE are three recipes for the **same dish**. Absorption (`| ID`) is just the fast one.]{.takeaway .fragment}
...
[Absorbing the year effect removes the aggregate wage trend FD's intercept was capturing. Time-invariant regressors (schooling, female) are *silently absorbed*.]{.takeaway .fragment}
...
[RE is a variance-weighted average of between and within. With only 9% within, it leans toward the *between* picture — and SE is $2.7\times$ tighter than FE.]{.takeaway .fragment}
...
[But $H$ has low power exactly when within variation is thin: a noisy FE inflates $V_{\mathrm{FE}}$ and shrinks $H$ mechanically.]{.takeaway .fragment}
...
[CRE within $= 0.2103$ (matches FE exactly); Mundlak term $\gamma = -0.144$, $p = 0.072$ — borderline, hinting at negative selection.]{.takeaway .fragment}
...
[Cross-sectional 7–11% · within ~21%. Standard errors swing inversely — the within camp is noisier but causally cleaner.]{.takeaway .fragment}
