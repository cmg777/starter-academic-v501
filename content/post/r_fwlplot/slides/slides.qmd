...
[Income opens a backdoor path coupons ← income → sales. Block it, or the naive slope is biased.]{.comment}
...
[A negative *raw* coupon–sales correlation, even though the true effect is positive — Simpson's paradox in one table.]{.comment}
...
[$M_{X_2}=I-X_2(X_2'X_2)^{-1}X_2'$ is the residual-maker: it strips the influence of the controls $X_2$ from whatever it multiplies.]{.comment}
...
[Every number here is a visual feature of the `fwl_plot()` scatter: the slope, and how tight the cloud is.]{.comment}
...
[Like comparing height *for your age*: judge each store's coupons and sales relative to its income level.]{.comment}
...
[$\hat\gamma$ = income's effect on sales; $\hat\delta$ = slope of coupons on income. True $+0.212$ plus bias $-0.148$ ≈ the naive $-0.093$.]{.comment}
...
[A race handicap: convert raw times into "faster or slower than your own average," then compare fairly.]{.comment}
...
[Within-route, longer-than-usual air time predicts *slightly less* delay — possibly tail-wind days.]{.comment}
