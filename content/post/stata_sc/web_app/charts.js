...
(function () {
  ...
  function donor_weight_animation(container) {
    const W = 720, H = 360;
    const svg = ensureSVG(container, W, H);
    // Final weights: Utah 33.4%, Nevada 23.5%, Montana 20.2%, Colorado 16.1%, Connecticut 6.8%, others 0
    const data = [
      { region: "Utah",        weight: 0.334, color: C.steel },
      { region: "Nevada",      weight: 0.235, color: C.orange },
      { region: "Montana",     weight: 0.202, color: C.orange },
      { region: "Colorado",    weight: 0.161, color: C.orange },
      { region: "Connecticut", weight: 0.068, color: C.orange },
      { region: "33 other donors (weight = 0)", weight: 0, color: C.muted },
    ];
    const margin = { top: 24, right: 24, bottom: 40, left: 200 };
    const w = W - margin.left - margin.right;
    const h = H - margin.top - margin.bottom;
    const g = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
  ...
  function paths_chart(container) {
    ...
    function update(data, opts) {
      ...
      if (opts.showGap) {
        ...
        yLabel.text("Gap = actual − synthetic  (packs per capita)");
      } else {
        const vals = [];
        ...
        yMin = d3.min(vals) - 5;
        yMax = d3.max(vals) + 5;
        yLabel.text("Cigarette sales  (packs per capita)");
      }
          ...
          if (opts.showActual) {
            linesG.append("circle").attr("cx", x(d.year)).attr("cy", y(d.actual))
              ...
              .on("mouseover", function (ev) {
                const rect = container.getBoundingClientRect();
                tooltip.html(
                  `<div><strong style="color:${C.orange}">Actual California</strong></div>` +
                  `<div><span class='tooltip-key'>year</span> <span class='tooltip-val'>${d.year}</span></div>` +
                  `<div><span class='tooltip-key'>cigsale</span> <span class='tooltip-val'>${d.actual.toFixed(1)}</span></div>` +
                  `<div><span class='tooltip-key'>gap</span> <span class='tooltip-val'>${d.gap.toFixed(2)}</span></div>`
                ).classed("show", true)
                  .style("left", (ev.clientX - rect.left + 12) + "px")
                  .style("top",  (ev.clientY - rect.top  + 12) + "px");
  ...
  function placebo_chart(container) {
    ...
    function update(data, opts) {
      ...
      g.append("text").attr("transform", `translate(${w / 2},${facetH + 32})`)
        .attr("text-anchor", "middle").attr("fill", C.text).attr("font-size", 12)
        .text("Post-1989 MSPE / Pre-1989 MSPE ratio");

      sorted.forEach((d, i) => {
        const yc = y(d.region);
        const isCalifornia = d.region === "California";
        const isRunnerUp   = d.region === "Georgia";
        const color = isCalifornia ? C.orange : (isRunnerUp ? "#9bdcc3" : C.steel);

        g.append("text").attr("x", -10).attr("y", yc + y.bandwidth() / 2 + 4)
          .attr("text-anchor", "end").attr("fill", isCalifornia ? C.orange : C.text)
        ...
        const rect = g.append("rect")
          .attr("x", 0).attr("y", yc).attr("height", y.bandwidth())
          .attr("width", Math.max(1, targetW))
          .attr("fill", color).attr("opacity", isCalifornia ? 0.95 : 0.65)
          .style("cursor", "pointer");
