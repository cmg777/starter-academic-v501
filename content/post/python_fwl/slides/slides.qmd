...
:::
::: {.column width="45%"}
[The path `coupons ← income → sales` is non-causal. Leave it open and the negative income→coupon link leaks into the coupon slope.]{.comment}
:::
::::
...
[We *plant* the answer (+0.2) in the data, then check whether each estimator finds it.]{.comment}
...
[Not just imprecise — the sign is *backwards* from the true +0.2. The confounder is pulling it down.]{.comment}
...
[Conditioning on income blocks the backdoor: the estimate jumps to +0.267, close to the true +0.2.]{.comment}
...
[Remove income from coupons, remove income from sales, then regress the leftovers. Same $\hat\beta_1$.]{.comment}
...
[Residuals are mean-zero, so we drop the intercept. The coefficient on `coupons_tilde` *is* the controlled effect.]{.comment}
...
[Same coefficient to four decimals; residualizing the outcome too restores the SE to match full OLS (0.120).]{.comment}
...
[Partial out income *and* day-of-week from both sides — identical coefficient. The theorem holds for any number of controls.]{.comment}
...
[Response.]{.rebuttal} FWL is pure algebra — it reproduces what OLS already computes, nothing more. The causal reading rests on the *assumption* that income is the only confounder (a correctly specified, linear backdoor). FWL makes that adjustment visible; it cannot certify it.
...
[Same residualize-then-regress logic; swap OLS for a flexible learner and you get a debiased causal estimate.]{.comment}
