...
subtitle: "Understanding the Frisch-Waugh-Lovell theorem to isolate causal relationships"
author: "Carlos Mendez"
date: "2026-03-14"
jupyter: python_fwl-tutorial
format:
  html:
    theme: cosmo
...
**1. Frisch-Waugh-Lovell theorem** $\hat\beta_1$ from full reg = $\hat\beta_1$ from residual reg. The full-regression coefficient on $X_1$ equals the simple regression of $\tilde Y$ on $\tilde X_1$, where the tildes are residuals from regressing on the other controls.
...
**2. Confounding** $X_2$ correlated with both $Y$ and $X_1$. A third variable creates a spurious link between regressor and outcome.
...
**4. Omitted variable bias** $\mathrm{OVB} = \gamma \cdot \delta$. The naive slope of $Y$ on $X_1$ differs from the true slope by exactly $\gamma \cdot \delta$, where $\gamma$ is the omitted variable's effect on $Y$ and $\delta$ is the slope of the omitted variable on $X_1$.
...
**5. Conditional vs marginal effect** $E[Y \mid X_1, X_2]$ vs $E[Y \mid X_1]$. The conditional effect holds other variables fixed. The marginal effect averages over them. The two are equal only when no relevant variable is omitted.
...
The figures in this tutorial use a dark theme for visual consistency with the published post. The code below sets the matplotlib `rcParams` once so every subsequent figure picks up the dark background, grid, and text colors automatically.

```{python}
#| label: imports-and-style
...
```{python}
#| label: fig-naive-regression
#| fig-cap: "Naive regression: the downward slope suggests coupons reduce sales, but this is driven by confounding from income."
...
3. **Full FWL:** Regress $\tilde{y}$ on $\tilde{x}_1$ (residuals of both variables on $x_2$)
...
```{python}
#| label: fig-residuals-income
#| fig-cap: "Partialling-out: the dashed lines are the residuals --- the coupon variation that income cannot explain."
...
ax.vlines(df["income"],
          ...
          linestyle="--", color=LIGHT_TEXT, alpha=0.5, linewidth=1,
          label="Residuals")
...
```{python}
#| label: fig-partialled-out
#| fig-cap: "After removing income's influence from both variables, the true positive effect of coupons on sales emerges."
...
```{python}
#| label: fig-scaled-residuals
#| fig-cap: "Adding the sample means back to the residuals restores interpretable units without changing the slope."
...
#| fig-cap: "Simpson's paradox resolved: the naive negative slope (left) reverses to a positive slope (right) after partialling out income."
...
If you want to see DML in action, check out the companion tutorial on [Introduction to Causal Inference: Double Machine Learning](https://carlos-mendez.org/post/python_doubleml/), which applies the partialling-out estimator to a real randomized experiment.
...
- See [Double Machine Learning](https://carlos-mendez.org/post/python_doubleml/) to learn how FWL extends to nonlinear settings.
- See [Introduction to Causal Inference: The DoWhy Approach](https://carlos-mendez.org/post/python_dowhy/) for a full causal inference workflow with real data.
...
- Companion script: [`script.py`](https://raw.githubusercontent.com/cmg777/starter-academic-v501/master/content/post/python_fwl/script.py)
- Published post: <https://carlos-mendez.org/post/python_fwl/>
