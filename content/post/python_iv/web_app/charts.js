...
(function () {
  ...
  function forest_plot(container) {
    ...
    const colorMap = {
      ...
      "OLS (full)":    C.orange,
      "OLS (base)":    C.orange,
      "OLS + continents": "#e89a7a",
      "PSL":           "#9bdcc3",
      "DL (rigorous)": C.teal,
      "DL (CV)":       C.orange,
      "IV (main, logem4)": C.teal,
      "IV + lat":          C.steel,
      "IV + colonial":     C.steel,
      "IV + religion":     C.steel,
      "IV + climate":      C.steel,
      "IV + ethnic frag":  C.steel,
      "IV + malaria":      "#9bdcc3",
      "IV + life exp.":    "#9bdcc3",
      "IV: euro1900":      "#c8d0e0",
      "IV: cons00a":       "#c8d0e0",
  ...
  // ------------------------------------------------------------------
  // IV concept animation (Tab 1 for IV posts).
  //   A flowing DAG: Z (instrument) -> X (endogenous regressor) -> Y (outcome),
  //   with U (unobserved confounder) contaminating X and Y. A particle travels
  //   the Z -> X -> Y path while a forbidden Z -> Y arrow is drawn dashed-red.
  // ------------------------------------------------------------------
  function iv_dag_animation(container) {
    const W = 720, H = 320;
    const svg = ensureSVG(container, W, H);

    // Node positions
    const Z = { x: 90,  y: 200, label: "Z", sub: "settler mortality", color: C.steel };
    const X = { x: 360, y: 200, label: "X", sub: "institutions",      color: C.orange };
    const Y = { x: 630, y: 200, label: "Y", sub: "log GDP",           color: C.teal };
    const U = { x: 495, y: 60,  label: "U", sub: "confounders",       color: "#1a3a8a" };

    const defs = svg.append("defs");
    defs.append("marker").attr("id", "arrow-teal").attr("viewBox", "0 0 10 10")
      .attr("refX", 9).attr("refY", 5).attr("markerWidth", 7).attr("markerHeight", 7)
      .attr("orient", "auto-start-reverse")
      .append("path").attr("d", "M0,0 L10,5 L0,10 z").attr("fill", C.teal);
    defs.append("marker").attr("id", "arrow-muted").attr("viewBox", "0 0 10 10")
      .attr("refX", 9).attr("refY", 5).attr("markerWidth", 7).attr("markerHeight", 7)
      .attr("orient", "auto-start-reverse")
      .append("path").attr("d", "M0,0 L10,5 L0,10 z").attr("fill", C.muted);
    defs.append("marker").attr("id", "arrow-red").attr("viewBox", "0 0 10 10")
      .attr("refX", 9).attr("refY", 5).attr("markerWidth", 7).attr("markerHeight", 7)
      .attr("orient", "auto-start-reverse")
      .append("path").attr("d", "M0,0 L10,5 L0,10 z").attr("fill", "#e76b6b");

    function arrow(p1, p2, color, dashed, marker, width) {
      svg.append("line")
        .attr("x1", p1.x).attr("y1", p1.y)
        .attr("x2", p2.x).attr("y2", p2.y)
        .attr("stroke", color)
        .attr("stroke-width", width || 2)
        .attr("stroke-dasharray", dashed || null)
        .attr("marker-end", marker ? `url(#${marker})` : null);
    }

    arrow({ x: Z.x + 28, y: Z.y }, { x: X.x - 28, y: X.y }, C.teal, null, "arrow-teal", 2.5);
    ...
    function node(d) {
      ...
      g.append("circle").attr("cx", d.x).attr("cy", d.y).attr("r", 28)
        .attr("fill", d.color).attr("stroke", "#141413").attr("stroke-width", 2);
      g.append("text").attr("x", d.x).attr("y", d.y + 6)
        .attr("text-anchor", "middle").attr("fill", "#fff")
        .attr("font-size", 20).attr("font-weight", 700).text(d.label);
      g.append("text").attr("x", d.x).attr("y", d.y + 50)
        .attr("text-anchor", "middle").attr("fill", C.text).attr("font-size", 11)
        .text(d.sub);
    }
    ...
    const particle = svg.append("circle")
      .attr("r", 7).attr("fill", C.orange)
      .attr("stroke", "#fff").attr("stroke-width", 2);

    function step(ts) {
      const phase = (ts / 1800) % 1;
      let cx, cy;
      if (phase < 0.5) {
        const t = phase / 0.5;
        cx = (Z.x + 28) + ((X.x - 28) - (Z.x + 28)) * t;
        cy = Z.y;
      } else {
        const t = (phase - 0.5) / 0.5;
        cx = (X.x + 28) + ((Y.x - 28) - (X.x + 28)) * t;
        cy = X.y;
      }
      particle.attr("cx", cx).attr("cy", cy);
      requestAnimationFrame(step);
    }
    requestAnimationFrame(step);
  }
  ...
  function ols_vs_iv_chart(container) {
    ...
    g.append("text").attr("x", w / 2).attr("y", h + 36)
      .attr("text-anchor", "middle").attr("fill", C.text).attr("font-size", 12)
      .text("X (endogenous regressor — institutions analogue)");
    g.append("text").attr("transform", `rotate(-90) translate(${-h / 2},${-42})`)
      .attr("text-anchor", "middle").attr("fill", C.text).attr("font-size", 12)
      .text("Y (outcome — log-GDP analogue)");

    function update(data) {
      const { points, ols_slope, ols_int, iv_slope, iv_int, beta_true } = data;
      ...
      xAxisG.call(d3.axisBottom(x).ticks(6).tickFormat(d3.format(".1f")))
        .selectAll("text").attr("fill", C.muted);
      yAxisG.call(d3.axisLeft(y).ticks(6).tickFormat(d3.format(".1f")))
        .selectAll("text").attr("fill", C.muted);
      g.selectAll(".domain, .tick line").attr("stroke", C.muted);

      const sel = pointsG.selectAll("circle").data(points);
      sel.exit().remove();
      sel.enter().append("circle").attr("r", 3.2).attr("opacity", 0.65)
        .merge(sel)
        .attr("cx", d => x(d[0]))
        .attr("cy", d => y(d[1]))
        .attr("fill", C.steel);
  ...
  // ------------------------------------------------------------------
  // IV vs OLS bias histograms (Tab 3 Run-100-sims).
  // ------------------------------------------------------------------
  function iv_vs_ols_histograms(container) {
    const W = 720, H = 260;
    const margin = { top: 18, right: 24, bottom: 38, left: 50 };
    const w = W - margin.left - margin.right;
    const h = H - margin.top - margin.bottom;
    ...
    function update(data) {
      g.selectAll("*").remove();
      const all = data.ols.concat(data.iv);
      if (all.length === 0) return;
      ...
      const nBins = 24;
      const bin = d3.bin().domain(x.domain()).thresholds(nBins);
      const binsO = bin(data.ols);
      const binsI = bin(data.iv);
      ...
      const meanO = d3.mean(data.ols);
      const meanI = d3.mean(data.iv);
      if (Number.isFinite(meanO)) {
        g.append("line").attr("x1", x(meanO)).attr("x2", x(meanO))
          .attr("y1", h * 0.15).attr("y2", h).attr("stroke", C.orange).attr("stroke-width", 1.5)
          .attr("stroke-dasharray", "3 3");
      }
      if (Number.isFinite(meanI)) {
        g.append("line").attr("x1", x(meanI)).attr("x2", x(meanI))
          .attr("y1", h * 0.15).attr("y2", h).attr("stroke", C.teal).attr("stroke-width", 1.5)
          .attr("stroke-dasharray", "3 3");
      }
      ...
      g.append("text").attr("transform", `translate(${w / 2},${h + 32})`)
        .attr("text-anchor", "middle").attr("fill", C.text).attr("font-size", 12)
        .text("β̂ across simulated datasets (orange = OLS, teal = IV)");
    }
    return { update };
  }
