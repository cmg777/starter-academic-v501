...
subtitle: "Computing causal bounds under unmeasured confounding with CausalBoundingEngine"
author: "Carlos Mendez"
date: "2026-03-13"
jupyter: python_partial_identification-tutorial
format:
  html:
    theme: cosmo
...
for pkg, want in PINNED.items():
    ...
    print(f"  {pkg:22s} {have:14s}  [{status}]")
```

We use [CausalBoundingEngine](https://pypi.org/project/causalboundingengine/), a Python package that provides a unified interface for applying and comparing multiple causal bounding methods.
...
```{python}
#| label: fig-observed-probs
#| fig-cap: "Observed job rates by training status showing 25.4% for untrained and 63.6% for trained workers."
...
#| fig-cap: "Horizontal interval chart comparing Manski, Autobound, and Entropy bounds on the ATE, with the true ATE marked as a dashed vertical line."
...
```{python}
#| label: fig-pns-bounds
#| fig-cap: "Horizontal interval chart comparing Tian-Pearl, Autobound, and Entropy bounds on the PNS."
...
```{python}
#| label: fig-coverage
#| fig-cap: "Bar chart showing 100% coverage rates for all three bounding methods across 100 simulations."
...
#| fig-cap: "Line plot showing Manski bound width stays constant at 1.0 across sample sizes, while Entropy bound width stays near 0.68 with decreasing variance."
...
This framework complements the point-identification methods covered in previous tutorials. [Double Machine Learning](https://carlos-mendez.org/post/python_doubleml/) and [DoWhy](https://carlos-mendez.org/post/python_dowhy/) assume all confounders are observed and produce precise estimates. Partial identification drops that assumption and produces bounds instead. The choice depends on whether the "no unmeasured confounders" assumption is credible in your application.
...
- Companion script: [`script.py`](https://raw.githubusercontent.com/cmg777/starter-academic-v501/master/content/post/python_partial_identification/script.py)
- Published post: <https://carlos-mendez.org/post/python_partial_identification/>
