...
(function () {
  ...
  // ==================================================================
  // PARTIAL IDENTIFICATION custom charts
  // ==================================================================

  // ------------------------------------------------------------------
  // Bounds-widening animation (Tab 1).
  //   Visualises three nested intervals: the naive point estimate (orange),
  //   the Entropy bounds (teal), and the wider Manski bounds (steel).
  //   The entropy band oscillates between "weakest assumption" (= Manski width)
  ...
  function bounds_widening_animation(container) {
    ...
    const barH = 22;
    const rows = [
      { name: "Point estimate (assumes no confounder)",   y: 8 },
      { name: "Entropy bounds (H(U|X,Y) <= theta)",       y: 8 + (barH + 18) },
      { name: "Manski bounds (no assumptions)",           y: 8 + 2 * (barH + 18) },
    ];
    rows.forEach(r => {
      g.append("text").attr("x", 0).attr("y", r.y - 4)
        .attr("fill", C.muted).attr("font-size", 11).text(r.name);
    });
    ...
    function step(ts) {
      if (t0 === null) t0 = ts;
      const elapsed = (ts - t0) / 1000;
      const phase = (Math.sin(elapsed * 0.5) + 1) / 2; // [0,1]
      // entropy width shrinks toward true ATE as phase -> 1
      const entLo = manskiLo + (trueATE - manskiLo) * phase;
      const entHi = manskiHi + (trueATE - manskiHi) * phase;
      const pointAlpha = Math.max(0, (phase - 0.6) / 0.4);
  ...
  // ------------------------------------------------------------------
  // Bounds forest plot (Tab 2). Faceted by outcome (ATE / PNS).
  //   data: array of { method, outcome, ci_lo, ci_hi, width, covers_truth, note }
  // ------------------------------------------------------------------
  function bounds_forest(container) {
    ...
    const colorMap = {
      "Manski":                   C.steel,
      "Autobound (LP)":           C.orange,
      "Entropy (theta = 0.1)":    C.teal,
      "Tian-Pearl (Closed Form)": C.steel,
    };

    function update(estimates, activeOutcomes, trueATE) {
      const outcomes = activeOutcomes.length ? activeOutcomes : ["ATE", "PNS"];
      const rows = estimates.filter(d => outcomes.includes(d.outcome));
      const nFacets = outcomes.length;
      const facetW = (W - margin.left - margin.right - (nFacets - 1) * facetGap) / nFacets;

      const methodsByOutcome = {};
      outcomes.forEach(o => {
        methodsByOutcome[o] = rows.filter(r => r.outcome === o).map(r => r.method);
      });
      const maxMethods = Math.max(1, ...Object.values(methodsByOutcome).map(m => m.length));
      const facetH = 38 * maxMethods + 24;
      const totalH = margin.top + facetH + margin.bottom;
      svg.attr("viewBox", `0 0 ${W} ${totalH}`);
      svg.selectAll("g.facet").remove();
      ...
      outcomes.forEach((outcome, oi) => {
        ...
        const xs = subset.flatMap(d => [d.ci_lo, d.ci_hi]);
        const xMin = Math.min(0, d3.min(xs));
        const xMax = Math.max(0, d3.max(xs));
        ...
        facet.append("line")
          .attr("x1", x(0)).attr("x2", x(0)).attr("y1", 0).attr("y2", facetH)
          .attr("stroke", C.faint).attr("stroke-dasharray", "3 4");

        if (outcome === "ATE" && Number.isFinite(trueATE)) {
          facet.append("line")
        ...
        if (oi === 0) {
          methods.forEach(m => {
            svg.append("text").attr("class", "facet-label")
              .attr("x", margin.left - 10)
              .attr("y", margin.top + y(m) + y.bandwidth() / 2 + 4)
              .attr("text-anchor", "end").attr("fill", C.text).attr("font-size", 12)
              .text(m);
        ...
        subset.forEach(d => {
          const yc = y(d.method) + y.bandwidth() / 2;
          const color = colorMap[d.method] || C.teal;
          const grp = facet.append("g").style("cursor", "pointer");
          grp.append("rect")
            .attr("x", x(d.ci_lo))
            .attr("y", yc - y.bandwidth() * 0.32)
            .attr("width", Math.max(2, x(d.ci_hi) - x(d.ci_lo)))
            .attr("height", y.bandwidth() * 0.64)
            .attr("fill", color).attr("opacity", 0.7).attr("rx", 3);
          [d.ci_lo, d.ci_hi].forEach(v => {
            grp.append("line")
              .attr("x1", x(v)).attr("x2", x(v))
              .attr("y1", yc - y.bandwidth() * 0.42)
              .attr("y2", yc + y.bandwidth() * 0.42)
              .attr("stroke", color).attr("stroke-width", 2);
          });
          grp.append("text")
            .attr("x", x(d.ci_hi) + 6).attr("y", yc + 4)
            .attr("fill", C.muted).attr("font-size", 10)
            .text(`w = ${d.width.toFixed(3)}`);

          grp.on("mousemove", function (ev) {
            const rect = container.getBoundingClientRect();
            tooltip.html(
              `<div><strong style="color:${color}">${d.method}</strong></div>` +
              `<div><span class='tooltip-key'>Lower =</span> <span class='tooltip-val'>${d.ci_lo.toFixed(4)}</span></div>` +
              `<div><span class='tooltip-key'>Upper =</span> <span class='tooltip-val'>${d.ci_hi.toFixed(4)}</span></div>` +
              `<div><span class='tooltip-key'>Width =</span> <span class='tooltip-val'>${d.width.toFixed(4)}</span></div>` +
              (d.covers_truth !== null && d.covers_truth !== undefined
                ? `<div><span class='tooltip-key'>Contains true ATE? </span> <span class='tooltip-val'>${d.covers_truth ? "yes" : "no"}</span></div>`
                : "") +
              `<div style="margin-top:6px;color:${C.muted};font-size:11px;">${d.note || ""}</div>`
            )
            .classed("show", true)
            .style("left", (ev.clientX - rect.left + 12) + "px")
            .style("top",  (ev.clientY - rect.top  + 12) + "px");
  ...
  // ------------------------------------------------------------------
  // Live-bounds bar (Tab 3 simulator).
  ...
  function live_bounds_bar(container) {
    ...
    function update(data) {
      ...
      const rows = [
        { name: "Manski",            color: C.steel, lo: data.manski.lo,  hi: data.manski.hi  },
        { name: "Entropy (θ = 0.1)", color: C.teal,  lo: data.entropy.lo, hi: data.entropy.hi },
      ];
      ...
      rows.forEach(r => {
        ...
        g.append("rect")
          .attr("x", x(r.lo))
          .attr("y", yc - y.bandwidth() * 0.32)
          .attr("width", Math.max(2, x(r.hi) - x(r.lo)))
          .attr("height", y.bandwidth() * 0.64)
          .attr("fill", r.color).attr("opacity", 0.7).attr("rx", 3);
        [r.lo, r.hi].forEach(v => {
          g.append("line").attr("x1", x(v)).attr("x2", x(v))
            .attr("y1", yc - y.bandwidth() * 0.42).attr("y2", yc + y.bandwidth() * 0.42)
            .attr("stroke", r.color).attr("stroke-width", 2);
        });
        g.append("text").attr("x", x(r.lo) - 4).attr("y", yc + 4)
          .attr("text-anchor", "end").attr("fill", C.text).attr("font-size", 11)
          .text(r.lo.toFixed(3));
        g.append("text").attr("x", x(r.hi) + 4).attr("y", yc + 4)
  ...
  // ------------------------------------------------------------------
  // Width-vs-sample-size line plot (Tab 4 sensitivity).
  ...
  function width_vs_n(container) {
    ...
    function update(rows) {
      ...
      const x = d3.scaleLog().domain([d3.min(ns), d3.max(ns)]).range([0, w]);
      const allW = rows.flatMap(r => [r.manski_width, r.entropy_width]);
      const y = d3.scaleLinear().domain([0, Math.max(1.1, d3.max(allW) * 1.1)]).range([h, 0]);

      g.append("line").attr("x1", 0).attr("x2", w).attr("y1", y(1)).attr("y2", y(1))
        .attr("stroke", C.faint).attr("stroke-dasharray", "3 4");

      const line = d3.line().x(d => x(d.n));

      g.append("path").datum(rows)
        .attr("fill", C.steel).attr("opacity", 0.15)
        .attr("d", d3.area().x(d => x(d.n))
          .y0(d => y(d.manski_width - d.manski_sd))
          .y1(d => y(d.manski_width + d.manski_sd)));
      g.append("path").datum(rows)
        .attr("fill", "none").attr("stroke", C.steel).attr("stroke-width", 2.5)
        .attr("d", line.y(d => y(d.manski_width)));
      rows.forEach(r => {
        g.append("circle").attr("cx", x(r.n)).attr("cy", y(r.manski_width)).attr("r", 4)
          .attr("fill", C.steel);
      });
      ...
      g.append("path").datum(rows)
        ...
        .attr("d", d3.area().x(d => x(d.n))
          .y0(d => y(d.entropy_width - d.entropy_sd))
          .y1(d => y(d.entropy_width + d.entropy_sd)));
      g.append("path").datum(rows)
        .attr("fill", "none").attr("stroke", C.teal).attr("stroke-width", 2.5)
        .attr("d", line.y(d => y(d.entropy_width)));
      rows.forEach(r => {
        g.append("rect").attr("x", x(r.n) - 4).attr("y", y(r.entropy_width) - 4)
          .attr("width", 8).attr("height", 8).attr("fill", C.teal);
      });

      g.append("g").attr("transform", `translate(0,${h})`)
        .call(d3.axisBottom(x).tickValues(ns).tickFormat(d => d.toString()))
        .selectAll("text").attr("fill", C.muted);
