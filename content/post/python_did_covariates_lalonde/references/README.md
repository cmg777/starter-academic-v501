# python_did_covariates_lalonde — Quarto project

Executable companion to the blog post:

> **Covariates in Difference-in-Differences: The LaLonde Test in Python**
> <https://carlos-mendez.org/post/python_did_covariates_lalonde/>

Reproduces Scott Cunningham's "Covariates, diff in diff and LaLonde test" in Python:
the ATT of a job-training program estimated eight ways, showing that covariates rescue
...
- `script.py` — the canonical companion script (full analysis with bootstrap SEs and all five figures).
- `README.md` — this file.

All data downloads automatically via the `causaldata` package — no local data files are needed.
...
The wrapper runs `setup_env.py` (creates a hermetic `.venv/`, installs pinned packages including `pyfixest`, `causaldata`, and `diff-diff`, registers the Jupyter kernel `python_did_covariates_lalonde-tutorial`) and then `quarto render tutorial.qmd`, finally opening `tutorial.html` in your default browser. First run takes ~2 minutes; subsequent runs are instant.

### Manual (terminal users)

1. Extract this ZIP anywhere on your machine.
2. Open a terminal *in the extracted `python_did_covariates_lalonde/` folder* and run:

   ```bash
   python3 setup_env.py
   ```

   This creates the `.venv/`, installs pinned packages, and registers a Jupyter kernel named `python_did_covariates_lalonde-tutorial`. The script is idempotent on re-runs.
...
- **Auto-relaunch:** if `python3` on your PATH is unsupported (e.g., Python 3.14 with no wheels, or 3.9 below the package minimum), `setup_env.py` scans your machine for a compatible Python 3.10–3.13 and relaunches itself with it. You'll see a `Note: ... Relaunching setup_env.py with it...` line — that's expected.
- **Windows:** if `python3` is not on PATH, use `python setup_env.py` instead.
- **Kernel not found:** if Render reports `Jupyter kernel 'python_did_covariates_lalonde-tutorial' not found`, you skipped the manual step — run `python3 setup_env.py` in a terminal and try again. The one-click wrapper avoids this entirely.
...
Online tutorial and full post: <https://carlos-mendez.org/post/python_did_covariates_lalonde/>
