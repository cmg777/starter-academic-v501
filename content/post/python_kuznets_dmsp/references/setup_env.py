...
into it, registers a named Jupyter kernel (`python_kuznets_dmsp-tutorial`) that
points at the venv's Python, and ensures the outer Python that Quarto invoked
has the minimal jupyter packages it needs to discover the kernel.
...
for free. (pyfixest 0.60 has no numba dependency, so no Intel-wheel pins are
needed.)
"""
...
# Direct imports from script.py (alphabetised), pinned to the versions the
# tutorial was verified against, plus the jupyter + ipykernel bootstrap entries
# needed for kernel registration.
PINNED: dict[str, str] = {
    "linearmodels": "7.0",
    "matplotlib":   "3.11.0",
    "numpy":        "2.4.6",
...
KERNEL_NAME = "python_kuznets_dmsp-tutorial"
KERNEL_DISPLAY = "Python Kuznets DMSP Tutorial"
...
def preflight() -> None:
    ...
    if version_unsupported:
        ...
        print(
            ...
            f"command will work on this Python. Use a different Python:\n"
            f"  - e.g. `~/miniforge3/envs/<env>/bin/python3 setup_env.py`\n"
            f"  - or install Python via python.org: https://www.python.org/downloads/"
            f"{suffix}",
            file=sys.stderr,
        )
    sys.exit(1)
