...
::: {.notes}
Two countries with the same national income can look completely different inside — one a booming capital ringed by poor hinterlands, the other broadly shared.
...
[Mean regional Gini $= 0.064$ (max $0.163$): most countries are internally fairly equal, with a long unequal tail.]{.takeaway .fragment}
...
::: {.notes}
Read the equation term by term: log regional income equals a baseline, plus the light elasticity β1 on log light, plus a near-one-for-one adjustment β2 for the country's income, plus geography, world-region and satellite effects.
...
## The calibrated light elasticity is 0.102, with regional income tracking national income almost one-for-one {background-color="#141413"}
...
![Predicted vs observed log regional GDP per capita, 5,258 region-years ($r = 0.925$). The fit holds from the poorest regions to the richest.](../python_kuznets_dmsp_06_predicted_vs_observed.png)

[The calibration generalises across the whole income range, not one band.]{.takeaway .fragment}
...
[Drop the absolute value and the Gini collapses to zero.]{.takeaway .fragment}

::: {.notes}
Broadcasting does the heavy lifting: y[:, None] - y[None, :] turns a length-n vector into an n×n matrix of all pairwise differences with no Python loop, and np.outer(w, w) weights each pair by both populations. The classic hand-coded-Gini trap is forgetting np.abs — the Gini sums the *size* of each gap |y_i − y_j|, not the signed difference; without it the positive and negative gaps cancel and the index is zero.
:::

## Population weights are not cosmetic — they correlate only 0.75 with equal weights

![Population-weighted vs equal-weight Gini across country-years (corr $= 0.75$). Most points sit below the 45° line: weighting lowers measured inequality by $0.0034$ on average.](../python_kuznets_dmsp_07_population_weights.png)
...
[$N = 879$, 180 countries, 5-year periods. Same sign pattern across all five indices — the N is not an artefact of the Gini.]{.takeaway .fragment}
...
[Up, then down, then faintly up — a description, not a policy lever.]{.takeaway .fragment}
...
## Beyond income, ethnic inequality is the strongest correlate — by far {background-color="#141413"}

[0.071]{.bignum}

[ethnic-Gini coefficient ($p < 0.001$, $N = 844$) · a $0.1$ higher ethnic Gini implies $+0.007$ regional Gini — about 11% of the $0.064$ mean]{.bignum-label}

::: {.notes}
Adding structural controls on top of the cubic + FE: countries where income differs sharply across ethnic homelands also have sharply unequal regions — the internal economic geography is bound up with the human geography. Resource rents push inequality up (+0.018, the resource curse), trade openness +0.005, aid/GDP +0.015; arable-land share pulls it down (−0.053) because dispersed farming equalises regions. The licensed-ICRG institutional-quality column cannot be reproduced and is omitted.
:::
...
![Conley spatial-HAC standard errors for the clean light elasticity ($\beta = 0.190$). The interval widens with the radius; the estimate does not move.](../python_kuznets_dmsp_12_conley_se.png)
...
[The point estimate $\beta = 0.190$ never moves; only the honest uncertainty grows.]{.takeaway .fragment}
...
::: {.notes}
And every income figure here is a prediction — accurate on average, wrong for any single unusual region.
