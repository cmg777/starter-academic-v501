// app.js — wires index.html DOM controls to CHARTS for the synthetic-control app.
...
(function () {
  ...
  // ------------------------------------------------------------------
  // Tab 1: Donor-weights bar animation.
  // ------------------------------------------------------------------
  ...
  function renderBalanceTable() {
    ...
    html += "<thead><tr>"
         + "<th>Predictor</th>"
         + "<th style='text-align:right;'>Treated Basque</th>"
         + "<th style='text-align:right;'>Synthetic Basque</th>"
    ...
    rows.forEach(r => {
      ...
      // Highlight when sample mean is far from treated (predictor where Basque
      // is a clear outlier in the donor pool).
      const outlier = Math.abs(r.treated - r.sample_mean)
                      > 0.4 * Math.abs(r.treated || 1);
  ...
  function renderGapStats() {
    ...
    document.getElementById("stat-att-avg").textContent = fmt(h.att_avg, 3);
    document.getElementById("stat-att-peak").textContent = fmt(h.att_peak_value, 3);
    document.getElementById("stat-att-end").textContent = fmt(
      state.data.gap_series[state.data.gap_series.length - 1].gap, 3
    );
    document.getElementById("stat-prefit").textContent = fmt(h.pre_mspe, 4);
  }
