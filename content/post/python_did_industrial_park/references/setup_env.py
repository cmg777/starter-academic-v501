...
into it, registers a named Jupyter kernel (`python_did_industrial_park-tutorial`)
that points at the venv's Python, and ensures the outer Python that Quarto
invoked has the minimal jupyter packages it needs to discover the kernel.
...
# write-quarto-notebook-python catalog: numba 0.63+ and llvmlite 0.46+ dropped
# macOS Intel wheels, so we pin to the last Intel-friendly pair. All probed on
# the dev machine (darwin/x86_64, Python 3.13).
PINNED: dict[str, str] = {
    "diff-diff":  "3.5.2",
    "matplotlib": "3.10.8",
    "numpy":      "2.3.5",
    ...
    "pyfixest":   "0.50.1",
    "scipy":      "1.17.1",
    # Last numba+llvmlite releases that ship macOS Intel wheels. pyfixest only
    # requires numba>=0.58.0, so this satisfies its constraint while avoiding
    # the source build on macOS Intel that newer numba would force.
    "llvmlite":   "0.45.0",
    "numba":      "0.62.1",
    "jupyter":    "1.1.1",
    "ipykernel":  "7.2.0",
...
KERNEL_NAME = "python_did_industrial_park-tutorial"
KERNEL_DISPLAY = "Industrial Park DiD Tutorial"
