...
# render wrappers, the data/ CSVs, or the bundle README.md changes, then commit
# the regenerated zip.
...
# Keep the bundle lean: no macOS cruft, no stray caches.
find "${STAGE_DIR}" -name ".DS_Store" -delete
find "${STAGE_DIR}" -name "__MACOSX" -type d -prune -exec rm -rf {} + 2>/dev/null || true

rm -f "${OUT_ZIP}"
( cd "$(dirname "${STAGE_DIR}")" && zip -rq "${OUT_ZIP}" "$(basename "${STAGE_DIR}")" -x '*.DS_Store' )
