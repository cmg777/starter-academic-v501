# Infographic Instructions — From DiD to SDID (Brexit)

Storyboard-first chalkboard infographic for the post `r_sc_dsc_sdid`. Four sections:
(A) full AI image prompt, (B) negative prompt, (C) condensed prompt, (D) panel
reference data for manual text overlay.

Template A (Causal Inference), layered panels — the message inventory carries seven
ON-IMAGE messages, so panels may add a sub-equation or annotation labels.
...
Panel 1 (top-left): Title "ONE UNITED KINGDOM" in steel blue small-caps. A chalk island outline splits into a solid version and a faint dotted twin drifting away from it, with a small ballot box between them and a dashed vertical line marked "2016". Callout in warm orange: "The control group never existed." Chalk arrow to Panel 2.
...
Panel 5 (bottom-center): Title "A FIRE DRILL FOR METHODS" in steel blue small-caps. A row of chalk smoke detectors of differing sizes, five faint white and one bold teal, mounted above a wavy chalk line of twenty small tick marks; the teal detector is visibly the smallest and quietest. Big warm orange callout: "0.0067 vs 0.0089." Chalk arrow to Panel 6.
...
### Panel 1 — One United Kingdom

- **Position**: Row 1, Column 1 (top-left)
- **Dramatic function**: Hook
- **Story beat**: "There is one United Kingdom, it took the treatment, and the version that stayed does not exist."
- **Callout**: "The control group never existed."
- **Key number**: N/A (memorable phrase panel)
- **Central sketch**: A chalk island outline splitting from a faint dotted twin, ballot box between them, dashed "2016" line.
- **Body sentences** (for manual overlay):
  - On 23 June 2016 the United Kingdom voted to leave the European Union.
  - Measuring the cost requires the UK that stayed — a country that exists in no dataset.
- **Transition to next**: "If the twin does not exist, build one."

### Panel 2 — Build One From Parts

- **Position**: Row 1, Column 2 (top-center)
- **Dramatic function**: Stakes
- **Story beat**: "Give 23 OECD economies weights so the blend tracks the real UK for 86 quarters."
- **Callout**: "23 donors, one recipe."
- **Key number**: N/A
- **Central sketch**: A cauldron blending 23 labelled droplets — three larger, tagged HUN, USA, JPN — into an outline matching a faint island.
- **Sub-equation** (in-panel, 80% opacity): "Σω = 1, ω ≥ 0"
- **Body sentences** (for manual overlay):
  - Quarterly log real GDP for 24 OECD economies, 1995Q1 to 2020Q4, leaves the UK treated and 23 donors.
  - Synthetic Britain is roughly one-fifth Hungary, one-fifth the United States, one-fifth Japan, one-sixth Canada.
  - The weights are shares: never negative, always summing to one.
- **Transition to next**: "But which recipe? There is more than one way to choose the weights."

### Panel 3 — A Ladder, Not a Method
...
- **Body sentences** (for manual overlay):
  ...
  - MASC blends matching with SC (2.73%); augmented SC drops non-negativity and adds a ridge leash (3.04%).
- **Transition to next**: "Why should weighting the quarters help at all?"

### Panel 4 — Two Ways to Be Wrong

- **Position**: Row 2, Column 1 (bottom-left)
- **Dramatic function**: Twist
- **Story beat**: "The bias splits in two, and no single weighting scheme kills both."
- **Callout**: "Fix one, the other remains."
- **Key number**: N/A (conceptual panel)
- **Central sketch**: A balance scale — left pan a roast turkey tagged "3.4 kg", right pan a curved line with a straight chord across it and the gap shaded.
- **Annotation labels**: "wrong row" (left), "the curve bends" (right)
- **Body sentences** (for manual overlay):
...
- **Story beat**: "Move the treatment date to twenty quarters when nothing happened; every estimate is then pure false alarm."
- **Callout**: "0.0067 vs 0.0089."
- **Key number**: **0.0067 vs 0.0089** (SDID's placebo error against plain synthetic control's)
- **Central sketch**: Six chalk smoke detectors above twenty tick marks; five faint white, the smallest one teal.
- **Body sentences** (for manual overlay):
  - Twenty artificial treatment dates from 2010Q1 to 2014Q4, each fitted on data up to that point only. The true effect is zero, so every estimate is error.
  - SDID is quietest at 0.0067 log points of root mean squared error, against 0.0089 for plain synthetic control.
  - The published ranking among the three SDID variants dissolves once all of them are graded at the same forecast horizon — part of their apparent gap was the harder exam, not the worse method.
- **Transition to next**: "And the answer?"

### Panel 6 — Report the Cloud
...
- **Body sentences** (for manual overlay):
  - Estimates run 2.73% to 3.06% at the end of 2018 and 3.83% to 4.20% a year later. Every one exceeds the 2.4% previously reported for this dataset.
  - Adding covariates makes the counterfactual worse, not better: with 86 pre-treatment outcomes already in the matching set, six extra predictors add only noise.
  - Uncertainty is wide. With 23 donors the smallest attainable permutation p-value is 0.042, and a 95% interval spans roughly 1% to 4.6%.
- **Transition to next**: N/A (story ends)

---

## Notes for the renderer

- **Three anchor numbers only** — Panels 3, 5 and 6. Panels 1, 2 and 4 carry phrases, so the orange
  reads as a signal rather than decoration.
- **No two panels share a metaphor type**: splitting island (Panel 1), blending vessel (Panel 2),
  ladder (Panel 3), balance scale (Panel 4), row of detectors (Panel 5), dartboard (Panel 6).
- **Panel 4 is the comparison panel** and uses a balance scale, with the two bias types weighed
  against each other rather than plotted.
- Do not attempt to draw the actual GDP series. Every chart in the post is a precise figure; this
  artifact is the story, not the evidence.
