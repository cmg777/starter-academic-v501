...
**Re-capture / fix a thumbnail**: `node scripts/capture-dashboard-screenshots.cjs --slug <slug>` regenerates just that one across all bundles. As of 2026-06-15 the `viirs-like-yearly` and `viirs-like-yearly-region` thumbnails render broken because their shared `viirs_like_annual` Earth Engine asset isn't publicly accessible — re-shoot those two once the asset is shared/republished on the GEE side.
